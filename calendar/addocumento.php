...
if ($profilSession == 4 || $profilSession == 3 || $profilSession == 1 || $profilSession == 8 || $profilSession == 10) {

	headerFunction("../projects/listprojects.php?&changeProject=true&" . session_name() . "=" . session_id());

}

if (($id == "" || $proje == "") && ($idcalender == "" || $idproje == "")) {

	headerFunction("../projects/listprojects.php?&changeProject=true&" . session_name() . "=" . session_id());

}

$tmpquery = "and SU.nu_seq_usuario = '$idSession'";
...
if ($action == "add") {
	...
	$tmpqueryy = "WHERE cal.id = '" . $idcalender . "'";
	$userCalender = new request();
	$userCalender->openCalendar($tmpqueryy);
	$data_Calendar = $userCalender->cal_date_end[0];

	if ($_FILES['mes']['name'] != "") {
		if ($_FILES['mes']['size'] > 1000000) {
			echo '<script>alert("arquivo fora do padrão (maior que 1000 Kbytes)!");history.back();</script>';
			exit ();
		}
		$tdem = 1;
		$arquivo = $_FILES['mes']['name'];
		$fo = $_FILES['mes']['tmp_name'];
		$fp = fopen($_FILES['mes']['tmp_name'], "rb");
		$img = fread($fp, filesize($_FILES['mes']['tmp_name']));
		fclose($fp);
		$img = addslashes($img);
		$tesdir = "../anexo/" . trim($idproje) . "/";
		$tesdir2 = "../anexo/" . trim($idproje) . "/reuniao/";
		$uploaddir = "../anexo/" . trim($idproje) . "/reuniao/" . trim($idcalender) . "/";
		//$diretorio = "D:/SGD2/SGD-2.5/erro/".$projectSession."/";
		$diretorio = "../anexo/" . trim($idproje) . "/reuniao/" . trim($idcalender) . "/";

		if (is_dir($tesdir)) {
			//echo "tem diretorio<br>"; 
		} else {
			//echo $newdir = $tesdir."reuniao/$idcalender/";
			if ($koo = mkdir("$tesdir", 0775)) {
				//echo "cria diretorio";

			} else {
				//echo "fail diretorio!".$tesdir." <br>";
				//exit;
			}
		...
		if (!is_dir($tesdir2)) {
			if ($k = mkdir("$tesdir2", 0775)) {
				//echo "cria diretorio";
			} else {
				//echo "fail diretorio!".$tesdir2." <br>";
				///exit;
			}
		}

		if ($ok = mkdir("$uploaddir", 0775)) {
			//echo "success ao criar diretorio <br>!";
		} else {
			// echo "fail diretorio! <br>";

		}

		//$fot = $fo;
		//$fot = $img ;
		$up = $diretorio . basename($_FILES['mes']['name']);
		//$up = $diretorio.basename($_FILES['mes']['name']);

		if (move_uploaded_file($_FILES['mes']['tmp_name'], $up)) {
			//echo "success $up ! <br>";
			move_uploaded_file($fot, $up);
			//$arg =  $up.$arquivo;
			$execute = "true";
		} else {
			//echo "Erro ao subir o ficheiro. Não se pode guardar o ficheiro.".$idproje."--".$idcalender;
			//.$up."".$_FILES['mes']['tmp_name']; 
			$execute = "false";
	...
	$dat = date('Y-m-d');
	//$tmpquery1 = "INSERT INTO ".$tableCollab["projects_corretiva"]." (descricao,erro,id_projeto) VALUES('$tit','$img','$projectSession')";
	$tmpquery1 = "INSERT INTO " . MYDATABASE . ".calender_reuniao (titulo, arquivo,id_calendar,id_projeto,data) VALUES('$tit','$up','$idcalender','$idproje','$dat')";
	if ($execute == "true") {
		connectSql("$tmpquery1");

		//ini envia email 
		$tmpmail = 'AND mem.profil IN(6) ';
		//exit;
		...
		for ($um = 0; $um < $contmail; $um++) {
			$envimail = new email();
			$envimail->enviar('Anexo de reunião ', 'Demanda numero ' . $idproje . ' <br> Foi anexado documento de reuniao. ', $usermail->mem_email_work[$um]);
			unset ($envimail);
	...
	//fim envia email 
	//headerFunction("../calendar/viewcalendar.php?id=".$idproje."&".session_name()."=".session_id());
	//headerFunction("home.php?id=$num&".session_name()."=".session_id());
	headerFunction("../calendar/viewcalendar.php?dateEnreg=$idcalender&dateCalend=" . $data_Calendar . "&type=calendDetail&" . session_name() . "=" . session_id());
	exit;
}
...
$bouton[6] = "over";
//$titlePage = $strings["support"];,".$strings["your_projectsite"].".
...
echo "<form method='POST' action='../calendar/addocumento.php?" . session_name() . "=" . session_id() . "&action=add&project=$projectSession#filedetailsAnchor' name='addsupport' enctype=\"multipart/form-data\" >";
$block1 = new block();
...
$tmquery_member = "AND mem.id =" . fixInt($projectDetail->pro_owner[0]);
$sel_member->openMembers($tmquery_member);
$nome_gestor = $sel_member->mem_name[0];
//----------$projectDetail->pro_owner[0];------------------
//-----------inicio ger rel------------------
$sel_grel = new request();
$tmquery_rel = "AND mem.id =" . fixInt($projectDetail->pro_mem_idpla[0]);
...
$tmquery_gpro = "AND mem.id =" . fixInt($projectDetail->pro_mem_idgpro[0]);
...
echo "<table cellpadding='0' cellspacing='0' border='0'>
		<tr align='left'><th colspan=\"2\"><strong style= color:'red';>".$strings["demanda"]."</strong></th></tr>
		<tr>
		...
		<tr>
			...
			<td align='right'>&nbsp;</td><td >&nbsp;$status[$idStatus]</td>
		</tr>
		<tr>
			<th nowrap class='FormLabel' valign='top' align='left'>&nbsp;&nbsp;&nbsp; ".$strings["date_open"]." : </th>
			<td align='right'>&nbsp;</td><td >&nbsp;".nl2br(createDate($projectDetail->pro_created[0],$timezoneSession))."</td>
		</tr>
		<tr>
			<th nowrap class='FormLabel' align='left'>&nbsp;&nbsp;&nbsp; ".$strings["priority"]." :</th>
			<td align='right'>&nbsp;</td><td >&nbsp;".$priority[$idPriority]."</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td align='right'>&nbsp;</td><td >&nbsp;".$nome_gestor."</td>
		</tr>
		<tr>
			<th nowrap class='FormLabel' align='left'>&nbsp;&nbsp;&nbsp; ".$strings["resp_relaciona"]." :</th>
			<td align='right'>&nbsp;</td><td >&nbsp;".$nome_g_rel."</td>
		</tr>
		<tr>
			<th nowrap class='FormLabel' align='left'>&nbsp;&nbsp;&nbsp; ".$strings["project_manager_permissions"]." :</th>
			<td align='right'>&nbsp;</td><td >&nbsp;".$nome_g_pro."</td>
		</tr>
		</table><br>";
*/
echo "<table cellspacing=\"0\" width=\"90%\" border=\"0\" cellpadding=\"3\">
<tr align='left'><th colspan=\"2\"><strong style= color:'red';>" . $strings["info_arquivo"] . "</strong></th></tr>";
/*
echo "<tr><th>".$strings["priority"]." :</th><td><select name=\"pr\">";
...
echo "</select></td></tr> */
echo "<tr><th>" . $strings["titulo"] . "</th><td><input size=\"32\" value=\"$tit\" style=\"width: 400px\" name=\"tit\" maxlength=\"99\" type=\"TEXT\"></td></tr>
<tr><th>" . $strings["arquivo"] . "</th><td><input size=\"32\" style=\"width: 400px\" name=\"mes\" maxlength=\"32\" type=\"file\"></td></tr>
<input type=\"hidden\" name=\"user\" value=\"$idSession\"><input type=\"hidden\" name=\"idcalender\" value=\"$id\"><input type=\"hidden\" name=\"idproje\" value=\"$proje\">
