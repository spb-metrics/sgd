...
$hora = date("H")-3;
$minu = date(':i');
$dateheure1 = $dateh." ".$hora.$minu;



$tmpquery = " AND mem.ID = '$idSession'";
$userDetail = new request();
$userDetail->openMembers($tmpquery);
$menber_org = $userDetail->mem_organization[0];

//Pega os sistemas 
//$tmpquerys = "AND UO.nu_seq_interno_uorg  = '".$userDetail->mem_organization[0]."'";
$tmpquerys = " ORDER BY NOME ";
//$tmpquerys = "WHERE sis.id_unidade = '".$userDetail->mem_organization[0]."'";
...
//$tmp2query = "WHERE mem.profil = '0'";
//$usersel = new request();
//$usersel->openMembers($tmp2query);
//$usermo = $usersel->mem_id;
...
if ($action == "add") {
	...
	if($select == "0" ){
	echo '<script>alert("Selecione a Fase!!");</script>';
	echo '<script>window.history.back();</script>';
	exit;
	}
	/*
	if(!isset($select) and ($checkbox=="0") ){
	$tdem = $checkbox ;
	}
	if($campofile=="1"){
	$tdem = 1 ;
...
/*	
if($_FILES['pl']['name'] !=""){
	if($_FILES['pl']['size'] > 1000000){
	echo '<script>alert("arquivo fora do padrão (maior que 1000 Kbytes)!");history.back();</script>';
    exit();
	}
	$tdem = 1 ;
	$fp = fopen($_FILES['pl']['tmp_name'],"rb");
	$img = fread($fp,filesize($_FILES['pl']['tmp_name']));
	fclose($fp);
	$img = addslashes($img);
...
$tmpquery = " WHERE fas.id_projeto='$projeto' ";
$testFase = new request();
$testFase->openFases($tmpquery);
$countFase = count($testFase->fas_id);
if($countFase <= 0){
	
	//PLANEJAMENTO
	//$tmpquery1 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('$dateh','$dateh','$dateh','$dateh','$projeto','1','$hoje','0')";
	$tmpquery1 = "INSERT INTO ".MYDATABASE.".FASES (ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('$projeto','1','$hoje','0')";
	connectSql("$tmpquery1");
	
	//REQUISITO
	//$tmpquery4 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('','','','','$projeto','2','$hoje','0')";
	connectSql("$tmpquery4");
	
	//DESENVOLVIMENTO
	$tmpquery5 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('','','','','$projeto','3','$hoje','0')";
	connectSql("$tmpquery5");
	
	//TESTE
	$tmpquery6 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('','','','','$projeto','4','$hoje','0')";
	connectSql("$tmpquery6");
	
	//HOMOLOGAÇÃO
	$tmpquery7 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('','','','','$projeto','5','$hoje','0')";
	connectSql("$tmpquery7");
	
	//TREINAMENTO
	$tmpquery8 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('','','','','$projeto','6','$hoje','0')";
	connectSql("$tmpquery8");
	
	//IMPLANTAÇÃO
	//$tmpquery2 = "INSERT INTO ".MYDATABASE.".FASES (DATA_INI_PLAN,DATA_FIM_PLAN,DATA_INI_REAL,DATA_FIM_REAL,ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('$dateh','$dateh','$dateh','$dateh','$projeto','7','$hoje','0')";
	$tmpquery2 = "INSERT INTO ".MYDATABASE.".FASES (ID_PROJETO,TIPO_FASE,DATA_FASE,STATUS_FASE) VALUES('$projeto','7','$hoje','0')";
	connectSql("$tmpquery2");
	
	if($select > 0 or $select!=1 or $select!=7 ){
	//COMBO SELECIONADO PELO USUÁRIO
	$tmpquery3  = " UPDATE ".MYDATABASE.".FASES SET DATA_INI_PLAN='$dateh',DATA_FIM_PLAN='$dateh',DATA_INI_REAL='$dateh',DATA_FIM_REAL='$dateh' WHERE ID_PROJETO='$projeto' AND TIPO_FASE='$select' ";
	...
	//exit;
		...
		for($z=0; $z < $countFase;$z++){
				if($testFase->fas_tipo[$z]==$select){
					if(data_invert($testFase->fas_data[$z]) < $dateh){
						$query = " ";
					
						if($testFase->fas_dat_ini_plan[$z]==""){
						$query .= " DATA_INI_PLAN = '$dateh', ";
						}
						if($testFase->fas_dat_fim_plan[$z]==""){
							if($query==""){
							$query = " DATA_FIM_PLAN = '$dateh' ";
							}else{
							$query .= " ,DATA_FIM_PLAN = '$dateh' ";
							}
						}
						if($testFase->fas_dat_ini_real[$z]==""){
							if($query==""){
							$query .= " DATA_INI_REAL = '$dateh' ";
							}else{
							$query .= " ,DATA_INI_REAL = '$dateh' ";
							}
						...
						//ALTERA FASE
						$tmpq  = " UPDATE ".MYDATABASE.".FASES SET $query WHERE id='".$testFase->fas_id[$z]."' ";
						//connectSql($tmpq);
					    //echo "achou".data_invert($testFase->fas_data[$z]) ;
				        // exit; 
					}
				
				}
			//echo "achou status - ".$testFase->fas_status[$z]." <br>select--".$select ;
			
		}
//exit; 
}	
			//FASE ESPECIFICA DO PROJETO
			$tmpquery = " WHERE fas.id_projeto='$projeto' AND TIPO_FASE='$select'";
			$tipoFase = new request();
			$tipoFase->openFases($tmpquery);
			$countFaset = count($tipoFase->fas_id);
			//
			$tmpquery1 = " INSERT INTO ".MYDATABASE.".S_RECURSO_ATIVIDADE (DESC_ATIVIDADE,NU_SEQ_FASE) VALUES('$pn','".$tipoFase->fas_id[0]."') ";
			connectSql("$tmpquery1");
			
			
			//ULTIMO RECURSO CADASTRADO
			$tmpquery = " WHERE NU_SEQ_FASE='".$tipoFase->fas_id[0]."' AND ROWNUM = 1 ORDER BY RA.NU_SEQ_ATIVIDADE DESC ";
			$recAtv = new request();
			$recAtv->openRecursoativ($tmpquery);
			$countrecAtv = count($recAtv->rec_id);
			//echo "<br>qtd".$countrecAtv."--id".$recAtv->rec_id[0] ;
			$recursoNew = $recAtv->rec_id[0];
			unset($recAtv);
			...
			$tmpquery = " AND ru.nu_seq_usuario='".$idSession."' ";
			$recUsu = new request();
			$recUsu->openRecurso_usuario($tmpquery);
			$countrecUsu = count($recUsu->rfunc_id);
			
			//echo $countrecUsu  ;
			if($recursoNew!="" && $_SESSION['id_pro']!="" && $countrecUsu > 0){ 
			
			
			$tmpquery = " INSERT INTO ".MYDATABASE.".S_RECURSO_PROJETO (nu_seq_recurso_usuario, nu_seq_projeto,dt_recurso_projeto, hr_recurso_projeto,nu_seq_atividade) VALUES('".$recUsu->rfunc_idrecurso[0]."','".$_SESSION['id_pro']."','$dateh','1','$recursoNew') "; 
			...
			//exit;
			/*
			$tmpquery = " ".MYDATABASE.".S_RECURSO_ATIVIDADE ";
			last_id($tmpquery);
			$projectNew = $lastId[0];
			echo "<br>projeto--".$projectNew;
			exit;
			unset($lastId);
			*/
						
			/*
			$tmpmail = "AND mem.profil = '6' " ;
			$usermail = new request();
			$usermail->openMembers($tmpmail);	
			$contmail = count($usermail);
			for($um=0; $um < $contmail ;$um++){
			//$enviar = "true";
			$envimail = new email();
			$envimail->enviar('Solicitação de Demanda','Demanda '.$projectNew.' -'.$pn.'  criado por:'.$nameSession.'',$usermail->mem_email_work[$um]);
			//$enviar = "false";
			//time(2);
			unset($envimail);
			}
	...
	//headerFunction("home.php?id=$num&".session_name()."=".session_id());
	headerFunction("recursocalendar.php?id=$project&updateProject=true&recurso=$recursoNew");
	exit;
}

//INI MOSTRA FASE
for($f=1 ; $f <= count($fase_array); $f++){
$fas_dado .= "<option value=\"".$f."\"  >".$fase_array[$f]."</option>";
}
//FIM MOSTRA FASE

$bouton[6] = "over";
//$titlePage = $strings["support"];
//titulo mostra usuario
$titlePage = $strings["welcome"]." $nameSession ";
//, ".$strings["your_projectsite"].".";
//include ("include_header.php");
include('../themes/'.THEME.'/header.php');

echo "<form accept-charset='UNKNOWN' onSubmit = 'return Validaatv(this);' method='POST' action='../calendar/addatividade.php?".session_name()."=".session_id()."&action=add&project=$projectSession#filedetailsAnchor' name='addsupport' enctype='multipart/form-data' >";

$block1 = new block();
$block1->heading($strings["nova_atividade"]);

echo "<table cellspacing=\"0\" width=\"90%\" border=\"0\" cellpadding=\"3\" >
<tr><th colspan=\"2\"></th></tr>
<tr><th width='120' align='left'>&nbsp;&nbsp;".$strings["phase"]." :</th><td><select name=\"select\" id='select' onChange=\"foco_campo('pn');\" style=\"width: 250px\" >
<option value=\"0\" >Selecione a Fase</option>";
  
echo $fas_dado;
//if(!$comptListSistem==""){
	//for($i=0;$i < $comptListSistem;$i++ ){
	//echo "<option value=\"".$listSistem->sis_id[$i]."\" >".$listSistem->sis_nome[$i]."</option>";
	//}
//} 
echo "</select>&nbsp;&nbsp; &nbsp;</td></tr>";

echo "<tr><th align='left'>&nbsp;&nbsp;".$strings["descricao"]." :</th><td><input size=\"32\" value=\"$pn\" style=\"width: 250px\" name=\"pn\" maxlength=\"70\" type=\"TEXT\"></td></tr>";
//echo "<tr id='cerro' style=\"display:none \"><th align='left'>&nbsp;&nbsp;".$strings["erro"]." :</th><td><input size=\"32\" value=\"$pl\" style=\"width: 250px\" name=\"pl\" maxlength=\"32\" type=\"file\"></td></tr>";
echo "<tr><th align='left'>&nbsp;&nbsp;</th><td>&nbsp;&nbsp;</td></tr>
 <tr><th colspan=\"2\"><br></th></tr>
<input type=\"hidden\" name=\"user\" value=\"$idSession\"><input type=\"hidden\" name=\"projeto\" value=\"$id\">
