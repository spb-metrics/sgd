...
//RECEBE PARÃMETRO                     
$Ufunc = $_REQUEST["uni"];    
//INI MOSTRA RECURSO ATIVIDADE
$tmpqueryg = " WHERE FAS.ID_PROJETO = '".$_SESSION['id_pro']."' AND FAS.TIPO_FASE = '".$Ufunc."'  ORDER BY RA.NU_SEQ_ATIVIDADE ASC";
//$tmpqueryg = " WHERE RA.NU_SEQ_FASE = '".$Ufunc."'  ORDER BY RA.NU_SEQ_ATIVIDADE ASC";
$recAtvg = new request();
$recAtvg->openRecursoativ($tmpqueryg);
$countrecAtvg = count($recAtvg->rec_id);
//echo "cont---".$countrecAtvg ;

//FIM MOSTRA RECURSO ATIVIDADE
...
if($countrecAtvg > 0) {                
   ...
   $xml .= "<atividades>\n";               
   	for($i=0; $i < $countrecAtvg; $i++){ 
	 // OCIExecute($result);
	    //$codigo = $res['ID'][$i];
	   // $descricao = $res['NAME'][$i];
	   $codigo = $recAtvg->rec_id[$i];
	   $descricao = $recAtvg->rec_descricao[$i];
	  $xml .= "<atividade>\n";     
