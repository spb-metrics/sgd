...
if ($action == "delete") {
	...
	for($i=0; $i < $countAtar; $i++ ){
	
		if($ok = unlink(trim($requestatar->atar_arquivo[$i]))){
		
		//echo "--sucesso";
		// exit;
		}else{
		
		//echo "nao encontrou".$requestatar->atar_arquivo[$i];
		 //exit;
		}
	...
	$tmpquery2 = "DELETE FROM ".$tableCollab["calender_reuniao"]." WHERE id_calendar IN($id)";
