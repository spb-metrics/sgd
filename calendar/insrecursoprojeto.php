...
if ($Ativ != "" && $Id != "" && $Horar != "" && $Datar != "" && $Recurso != "" && $Fase != "") {

	//ini pega atividade
	$tmpqueryg = "  WHERE RA.nu_seq_atividade='" . $Ativ . "'";
	$recAtvg = new request();
	$recAtvg->openRecursoativ($tmpqueryg);
	$countrecAtvg = count($recAtvg->rec_id);
	$id_tipofase = $recAtvg->rec_fase[0];
	//pega fase
	$tmpquery = " WHERE fas.id='" . $id_tipofase . "' ";
	$listFases = new request();
	$listFases->openFases($tmpquery);
	$countFase = count($listFases->fas_id);
	$id_fase = $listFases->fas_id[0];
	//-- verifica tipo da fase 7, data da fase igual a hoje, se a data de recurso é menor que data de inicio do planejamento --\\ 		
	if ($recAtvg->rec_fase_tipo == 7 && $listFases->fas_data[0] == $hoje) {

		if ($Datar < $listFases->fas_dat_ini_plan[0]) {
			//atualiza a data de inicio do planejamento e inicio realizado
			$tmpp = "UPDATE ".MYDATABASE."." . $tableCollab["fases"] . " SET data_ini_plan='" . $Datar . "', data_fim_plan='" . $Datar . "', data_ini_real='" . $Datar . ", data_fim_real='" . $Datar . "' WHERE id = '" . $id_tipofase . "' ";
			connectSql("$tmpp");
		}
	...
	$tmpquery = " INSERT INTO ".MYDATABASE.".S_RECURSO_PROJETO (nu_seq_recurso_usuario, nu_seq_projeto,dt_recurso_projeto, hr_recurso_projeto,nu_seq_atividade) VALUES('$Recurso','$Id','" . $Datar . "','$Horar','$Ativ') ";
	connectSql("$tmpquery");

	$tmpqueryy = " ORDER BY RPRO.nu_seq_rec_proj DESC ";
	$listProjects = new request();
	$listProjects->openRecprojeto($tmpqueryy);
	$Num = $listProjects->recpro_id[0];
	//echo $Num ;
	...
	if ($countFase > 0) {
		...
		//INI FASE VAZIA
		$qry = " ";
		if ($listFases->fas_dat_ini_plan[0] == "") {
			//atualiza data
			$qry .= " data_ini_plan='" . $Datar . "', ";
		}
		if ($listFases->fas_dat_fim_plan[0] == "") {
			//atualiza data
			$qry .= " data_fim_plan='" . $Datar . "', ";
		}
		if ($listFases->fas_dat_ini_real[0] == "") {
			//atualiza data
			$qry .= " data_ini_real='" . $Datar . "', ";
		}
		if ($listFases->fas_dat_fim_real[0] == "") {
			//atualiza data
			$qry .= " data_fim_real='" . $Datar . "' ";
		}

		$tmp = "UPDATE " . $tableCollab["fases"] . " SET $qry WHERE id = '" . $id_tipofase . "' ";
		connectSql("$tmp");
		...
		//INI FASE ANTIGA
		//--se nao for no mesmo dia da criação da fase, impossivel alterar planejamento--
		$qry2 = " ";
		if ($listFases->fas_data[0] == $hoje) {
			if ($Datar > $listFases->fas_dat_fim_plan[0]) {
				//atualiza data fim planejado
				$qry2 .= " data_fim_plan='" . $Datar . "', ";
			}
		}

		if ($Datar > $listFases->fas_dat_fim_real[0]) {
			//atualiza data fim realizado
			$qry2 .= " data_fim_real='" . $Datar . "' ";
		}

		$tmp2 = "UPDATE ".MYDATABASE."." . $tableCollab["fases"] . " SET $qry2 WHERE id = '" . $id_tipofase . "' ";
		connectSql("$tmp2");

		//Fase Implatação
		$tmp22 = "UPDATE ".MYDATABASE."." . $tableCollab["fases"] . " SET $qry2 WHERE id_projeto = '" . $Id . "' AND tipo_fase = '7' ";
		connectSql("$tmp22");
...
$tmp_Rfunc = " AND RU.NU_SEQ_USUARIO IS NOT NULL AND RU.nu_seq_sub_funcao_sgd = '".$Ufunc."' ORDER BY SC.NO_CADASTRO ";
...
if ($contRfunc > 0) {
	...
	$Dia_es = substr("$Datar", 8, 1);
	if ($Dia_es == 0) {
		$Dia_esp = substr("$Datar", 9, 1);
	} else {
		$Dia_esp = substr("$Datar", 8, 2);
	}

	$xml .= "<recurso>\n";
	$xml .= "<acao>1</acao>\n";
	$xml .= "<status>" . $contRfunc . "</status>\n";
	$xml .= "<data>" . invert_data($Datar) . "</data>\n";
	$xml .= "<datars>" . $Datar . "</datars>\n";
	$xml .= "<horas>" . $Horar . "</horas>\n";
	$xml .= "<projeto>" . $Id . "</projeto>\n";
	$xml .= "<recpro>" . $Num . "</recpro>\n";
	$xml .= "<diaesp>" . $Dia_esp . "</diaesp>\n";
	$xml .= "<query>" . $tmp2 . "</query>\n";
