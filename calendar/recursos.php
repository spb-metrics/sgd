...
$tmp_Rfunc = " AND RU.NU_SEQ_USUARIO IS NOT NULL AND RU.nu_seq_sub_funcao_sgd = '".$Ufunc."' ORDER BY SU.login";
...
if($contRfunc > 0) {                
   ...
   $xml .= "<gestores>\n";               
   	for($i=0; $i < $contRfunc; $i++){ 
	 // OCIExecute($result);
	    //$codigo = $res['ID'][$i];
	   // $descricao = $res['NAME'][$i];
	   $codigo = $all_Rfuncao->rfunc_id[$i];
	   $descricao = $all_Rfuncao->rfunc_nome[$i];
	  $xml .= "<gestor>\n";     
