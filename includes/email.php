...
class email {
	...
	function email() {
		$this->remetente = '<SGD@fnde.gov.br>';
		
		$this->corpo ='<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
"http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>SGD - Informa</title>
...
body {
	...
	margin-right: 0px;
	margin-bottom: 0px;
	background-image:  url(http://intranet:81/sgd/themes/default/images/bg_grey.gif);
}
-->
</style>
<link href="http://intranet:81/sgd/themes/default/stylesheet2.css" rel="stylesheet" type="text/css">
</head>

<body>
<table width="640" border="0" cellpadding="0" cellspacing="0" class="bordas" align="center">
  <tr>
    <td><img src="http://intranet:81/sgd/themes/default/images/topo_email.jpg" width="640" height="92" border="0" usemap="#Map"></td>
  </tr>
  <tr>
    <td bgcolor="#FFFFFF"><table width="640" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td>&nbsp;</td>
        <td class="textotopos">SGD - Informa </td>
        <td>&nbsp;</td>
      </tr>
      <tr>
        <td>&nbsp;</td>
...
//<A name=loginAnchor></A>
        /*    $this->corpo2 ='<BR><P id=footer>SGD v1.0</P></TR></TABLE>
  <TR bgColor=#006699>
    <TD align=center colSpan=3 ><SPAN style="COLOR: #ffffff font-size:10px;">FNDE 2006 - Fundo 
      Nacional de Desenvolvimento a 
Eduacação</SPAN></TD>
...
<P></P></BODY></HTML>'; */
  ...
  <tr>
    <td bgcolor="#006699"><div align="center" class="basefnde">&copy; FNDE 2006 - Fundo Nacional de Desenvolvimento da Educa&ccedil;&atilde;o </div></td>
  </tr>
</table>
<map name="Map">
  <area shape="rect" coords="598,5,635,30" href="#" alt="Entrar no SGD">
  <area shape="rect" coords="550,5,591,31" href="#" alt="Ajuda?">
</map>
</body>
</html>
';
	...
	// Envia para usuário
	function enviar($assunto,$message,$mail){
	...
	$msg.= "$message";
	$msg.= "$this->corpo2";
	$ma='lcerqueira@fnde.gov.br';
	//$ma='mnascimento@fnde.gov.br';
	//$ma = "$mail";
	mail("$ma","$assunto","$msg","$headers");
	
	}
	// Envia para Grupo
	function enviarGrupo($grupo,$assunto,$message){
	$contg = count($grupo);
		for($i=0;$i < $contg; $i++){
		$user = $grupo[$i];
		$headers = "Content-Type: text/html; charset=iso-8859-1\n";  
		...
		$mail="$user";
		 if(@mail("$mail","$assunto","$message","$headers")){
		 }else{ echo "<script>alert('nao envia!!!');</script>";}
		}
	}
