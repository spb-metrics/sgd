...
class arquivo {
	...
	function gera($data,$hora,$ctl){
	 $this->dat = $data;
	 $this->hora = $hora;
	 $this->tesdir = "../anexo/log/";
	 $this->tesdir2 = "../anexo/log/".$data."/";
	 $this->ctl = $ctl;
	
	
	if(is_dir($this->tesdir)){ 
	//echo "tem diretorio<br>"; 
	}else{
			
			if ($koo = mkdir ($this->tesdir, 0775) ){
			}else{			
			    //echo "fail diretorio!".$tesdir." <br>";
			     }
        } 
			
				if(!is_dir($this->esdir2)){ 
							if ($k = mkdir ($this->tesdir2, 0775) ){
							}else{
	...
	//################-GERA ARQUIVO-####################	
	$arquiv = $this->tesdir2.$this->dat.$this->hora.$this->ctl.".txt";
	$han = fopen("../anexo/".$arquiv, "x");
	...
	//##############-GERA ARQUIVO DE NOME-####################
	$this->diretorio = "../anexo/";
	$this->arqui = $arquivo;
	...
	//echo "tem diretorio<br>"; 
			...
			if ($koo = mkdir ($this->diretorio, 0775) ){
			}else{			
			    //echo "fail diretorio!".$this->diretorio." <br>";
			     }
        } 

	//################-GERA ARQUIVO-####################	
	$arquiv = $this->diretorio.$this->arqui.".txt";
	$han = fopen($arquiv, "x");

	} //function geraNome
	
	
	function insert ($dado,$dir){
	//################-INSERE DADO NO ARQUIVO -####################	
	$gravar = fopen($dir, "a");
	$grava = fwrite($gravar, $dado);
	...
	$open = fopen($dir, "r");
	//$open = fwrite($gravar, $dado);
	$ler = fread($open, filesize($dir));

	 return  $ler ;
	
	}  // function abrir
	
	function referencia($d,$st,$id,$ds,$md){
	//################- GERA REFERENCIA NO BANCO -####################	
	//$open = fopen($dir, "r");
	//$open = fwrite($gravar, $dado);
	//$ler = fread($open, filesize($dir));
   $tmpquery2 = "INSERT INTO ".MYDATABASE.".s_control_in (an_date, st_status, id_projects,ds_processo,md_documento) VALUES('$d','$st','$id','$ds','$md')";
   // exit;
