...
if (($profilSession == 0) || ($profilSession == 10) || ($profilSession == 9) || ($profilSession == 7) || ($profilSession == 6) || ($profilSession == 8) || ($profilSession == 3)){
$blockHeadera = new block();
echo '<table width="100%" border="0" cellspacing="0" cellpadding="0"><tr>';
  
      if ($profilSession == 0 || $profilSession == 6 || $profilSession == 7 || $profilSession == 10 ){
	   echo '<td width="1%">&nbsp;</td>';
       echo "<td width=\"10%\">".$blockHeader->buildLink("../projects/listprojects.php?fillter=true&fill=",$strings["demandas"],in,"Mostrar Demandas")."</td>";
       echo '<td align="left" width="5%">|</td>'; 
		}
	 
	  echo "<td width=\"10%\">".$blockHeader->buildLink("../reports/index.php?",$strings["reports"],in)."</td>";
      if (($profilSession == 0) || ($profilSession == 6) || ($profilSession == 10) || ($profilSession == 7)){
	  echo '<td align="left" width="5%">|</td>';
	  echo "<td width=\"10%\">".$blockHeader->buildLink("../administration/admin.php?",$strings["admin"],in)."</td>";
	  
	  }
	    echo '<td align="center" width="5%">|</td>';
	  echo "<td width=\"10%\">".$blockHeader->buildLink("../projects/listprojects.php?situa=10","Histórico",in)."</td>";
	
	 // echo $block1->buildLink("../projects/listprojects.php?situa=10","Demandas Concluidas",in);
	  echo '<td id="account1" width="36%" align="right">';
	  


//$blockHeadera->openAccount();
	...
	//$blockHeader->itemAccount("&nbsp;");
		...
		//$blockHeader->itemAccount(
		echo $strings["user"].": ". $nameSession."&nbsp;&nbsp;|";
		//);
		//$blockHeader->itemAccount("|");
		//$blockHeader->itemAccount(
		echo $blockHeadera->buildLink("../general/login.php?logout=true",$strings["logout"],in);
		echo "&nbsp;|";
		//$blockHeader->itemAccount("|");
