...
<html>
<head>
<title>Incluir Sistemas/Gestores no SGD</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">

<script language="JavaScript" type="text/javascript">
function checa(form){
	
	 var gestor = form.text_gestor.value;
	 if (gestor == ""){
		alert("Selecione o Gestor!!!");
		form.text_gestor.focus();
		return false;
	 }
...
//}
 ...
 $sigla = $_REQUEST['sistema'];
 $id_gestor = $_REQUEST['text_gestor'];
 $novasigla = $_REQUEST['novo_gestor'];
 $novo = $_REQUEST['novo'];
if($novo == 1){
   if($id_gestor!="" && $novasigla!=""){
      $sql = "SELECT ID_SISTEMA ID, CO_SISTEMA SG FROM ".MYDATABASE.".SISTEMAS WHERE CO_SISTEMA = '".strtoupper(trim($novasigla))."'";
	...
	if($row > 0){
	  ...
	  While(OCIFetch($result)){
               $id_sistema = OCIResult($result, 'ID');
         }
             if($id_sistema == 0){
                 $id_sistema = "";
              } 
    ...
    if($id_sistema !=""){
	$sql2 = "INSERT INTO ".MYDATABASE.".SISTEMAS (ID_SISTEMA, CO_SISTEMA, NU_SEQ_GESTOR_APLICACAO) VALUES ('".$id_sistema."','".strtoupper(trim($novasigla))."','".$id_gestor."' )";
	$RESULT = OCIParse($con, $sql2);
	OCIExecute($RESULT);
	echo "<script> alert('Dados Enviados!');</script>";
	}else { 
	$sql2 = "INSERT INTO ".MYDATABASE.".SISTEMAS (CO_SISTEMA, NU_SEQ_GESTOR_APLICACAO) VALUES ('".strtoupper(trim($novasigla))."','".$id_gestor."')";
	$RESULT = OCIParse($con, $sql2);
	OCIExecute($RESULT);
	echo "<script> alert('Dados Enviados!');</script>";
       }               	 
	  ...
	  if($id_gestor!="" && $sigla!=""){
      	     $sql = "SELECT ID_SISTEMA ID, CO_SISTEMA SG FROM ".MYDATABASE.".SISTEMAS WHERE CO_SISTEMA = '".$sigla."'";
	     $result = OCIParse($con, $sql);
	     OCIExecute($result);
	     $row = OCIFetchStatement($result, $res);
	     if($row > 0){
      ...
      if($id_sistema !=""){
	 $sql2 = "INSERT INTO ".MYDATABASE.".SISTEMAS (ID_SISTEMA, CO_SISTEMA, NU_SEQ_GESTOR_APLICACAO) VALUES ('".$id_sistema."','".$sigla."','".$id_gestor."' )";
	 $RESULT = OCIParse($con, $sql2);
	 OCIExecute($RESULT);
	 echo "<script> alert('Dados Enviados!');</script>";
	}else{ 
	 $sql2 = "INSERT INTO ".MYDATABASE.".SISTEMAS (CO_SISTEMA, NU_SEQ_GESTOR_APLICACAO) VALUES ('".$sigla."','".$id_gestor."')";
	 $RESULT = OCIParse($con, $sql2);
	 OCIExecute($RESULT);
	 echo "<script> alert('Dados Enviados!!');</script>";
	}               	 
...
<form name="form1" method="POST" action="registra.php" onSubmit="return checa(this);">
  <table width="499" height="210" border="0" cellpadding="0" cellspacing="0">
    <tr align="center"> 
      <td colspan="3"><strong>Incluir Sistemas/Gestores no SGD</strong></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <tr> 
      ...
      <td> 
        <?php 
              $sql = "SELECT CO_SISTEMA SIGLA FROM SEGURANCA_ADM.S_SISTEMA ORDER BY CO_SISTEMA";
   		 $result = OCIParse($con, $sql);
		 OCIExecute($result);
		 $row = OCIFetchStatement($result, $res);
        ...
        <select name="sistema" >
          ...
          <?php for($i=0; $i < $row; $i++){ ?>
          <option value="<?php echo $res['SIGLA'][$i]; ?>"> 
          <?php echo $res['SIGLA'][$i]; ?> </option>
          <?php } ?>
        </select></td>
    </tr>
    <tr> 
      ...
      <td> 
        <?php 
              //$sql2 = "SELECT NU_SEQ_USUARIO ID, DS_LOGIN LOGIN FROM SEGWEB_FNDE.S_USUARIO ORDER BY LOGIN";
   		 $sql2 = "SELECT DISTINCT(SU.nu_seq_usuario) ID, SU.ds_login LOGIN
			...
			LEFT OUTER JOIN SEGWEB_FNDE.S_USUARIO_INTERNO SUI ON SUI.NU_SEQ_USUARIO=SU.NU_SEQ_USUARIO
			...
			WHERE SA.SG_APLICACAO = 'SGD' AND SUG.ST_ATIVO='S' AND SG.NO_GRUPO = 'sgd_gestor' AND SCL.DT_SAIDA_LOTACAO IS NULL ORDER BY LOGIN";
               $result2 = OCIParse($con, $sql2);
		 OCIExecute($result2);
		 $row2 = OCIFetchStatement($result2, $res2);
		 
	    ?>
        <select name="text_gestor" >
          <option value="">Selecione o Usuário/Gestor</option>
          <?php for($i=0; $i < $row2; $i++){ ?>
          <option value="<?php echo $res2['ID'][$i]; ?>"> 
          <?php echo $res2['LOGIN'][$i]; ?></option>
          <?php } ?>
        </select></td>
    </tr>
    <tr> 
    ...
    <tr> 
      <td>&nbsp;</td>
      <td>Cadastrar um Sistema: 
        <input onChange="enableField(this.value)" type="radio" name="novo" id="novo" value="1"></td>
      <td>Sigla do Sistema: 
        <input type="text" name="novo_gestor" id="novo_gestor" disabled></td>
    </tr>
    <tr> 
      <td width="3%">&nbsp;</td>
      <td width="33%">&nbsp;</td>
...
function enableField (valor) {
  if (valor == '1') document.getElementById('novo_gestor').disabled = false;
  else document.getElementById('novo_gestor').disabled = true;
  if (valor == '1') document.getElementById('sistema').disabled = true;
  else document.getElementById('sistema').disabled = false;
}
</script>
</body>
</html>