...
include_once('../includes/library.php');
  ...
  if($action == "cbx"){
    ...
    if($dateStart!="" && $descricao!="" ){
			   
            $tmpq1 = " INSERT INTO ".MYDATABASE.".s_recurso_feriado (ds_feriado, dt_feriado) VALUES('".$descricao."','".data_invert($dateStart)."') ";
	     connectSql("$tmpq1");
	
	 echo "<script>window.alert('Cadastrado com sucesso!');</script>";	
	
	 echo "<script>window.location.href='../administration/recursoferiado.php?';</script>";

        exit;
   }else{
	 
	 echo "<script>window.alert('Preencha os campos nescessários!');window.history.back();</script>";	
  ...
  if($action == "update"){
           ...
           $tmpq1 = " UPDATE ".MYDATABASE.".s_recurso_feriado SET ds_feriado='".$descricao."', dt_feriado='".data_invert($dateStart)."' WHERE nu_seq_feriado='".$idctl."'";
	    connectSql("$tmpq1");
	
      echo "<script>window.alert('Atualizado com sucesso !');</script>";
      echo "<script>window.location.href='../administration/recursoferiado.php?';</script>";
      //headerFunction('../administration/recursoferiado.php?did='.$idctl.'&'.session_name().'='.session_id());
   exit;
  }
 ...
 include('../themes/'.THEME.'/header.php');
  ...
  $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/recursos.php?",$strings["recurso"],in));
  $blockPage->itemBreadcrumbs("<strong>Feriado</strong>");
  $blockPage->itemBreadcrumbs($strings["admin_intro"]);
  $blockPage->closeBreadcrumbs();

  if($type="dispensaEdit" && $did!=""){
    	       
              //INI MOSTRA FERIADO
	
		$tmp_recf = " WHERE recf.nu_seq_feriado = '".$did."'  ";
		$uni_Recf = new request();
		$uni_Recf->openRecferiado($tmp_recf);
		$contRecf = count($uni_Recf->recf_id);

   echo "<br><form  method='POST' action='recursoferiado.php?idusu=".$idusu."&action=update&".session_name()."=".session_id()."&' name='addsup' enctype='multipart/form-data'  >";
             $acao = "Editar";
	      $botton =  "<a href='recursoferiado.php?'><img  id=\"admm\" src=\"../themes/".THEME."/btn_add_norm.gif\" alt=\"Nova Função\" border=\"0\" onmouseover=\"trocaImg('btn_add_over.gif','admm');\" onmouseout=\"trocaImg('btn_add_norm.gif','admm');\" style=\"display:''\"></a>";
            }else{
	      $acao = "Registrar";
   echo "<br><form  method='POST' action='recursoferiado.php?idusu=".$idusu."&action=cbx&".session_name()."=".session_id()."&' name='addsup' enctype='multipart/form-data' >";
	      $botton = "";
	     }
	    		
          $block1 = new block();
	   $block1->heading($strings["feriado_management"]);
   echo $botton; 
   echo '<table width="99%" border="0" cellspacing="0" cellpadding="0" class="content">';
	
          $block1->contentSub($acao,"4") ;
   ...
   echo '<tr class="odd">
    ...
    echo "<tr class='odd'>
              <td width=\"33%\" align=\"right\" height=\"25\" valign=\"top\">".$strings["date"].": </td>
	       <td width=\"5%\">&nbsp;</td>
	       <td width=\"42%\" align=\"left\"><input type='text' name='dateStart' id='sel1' size='20' value='".invert_data($uni_Recf->recf_data[0])."'><input type='reset' value=' ... ' onclick=\"return showCalendar('sel1', '%d-%m-%Y');\"></td>
              <td width=\"20%\" ></td>
         </tr>";
         //

    echo '<tr class="odd"><td  align="right" valign="top">&nbsp;Descrição: </td>
              <td align="center">';
	           echo "<input id=\"control\" type=\"hidden\" name=\"control\" value=\"\">";
	           echo "<input id=\"idctl\" type=\"hidden\" name=\"idctl\" value=\"".$uni_Recf->recf_id[0]."\">";
	  echo '</td>';
	  echo "<td height=\"50\" valign=\"top\"><input type='text' name='descricao' id='sel12' maxlength='29' size='20' value='".$uni_Recf->recf_descricao[0]."'></td>";
	  echo "<td align=\"center\">&nbsp;</td>";
    echo' </tr>
         <tr class="odd">
             <td height="30">&nbsp;</td>
             <td align="center" >';
	          echo "<input type=\"submit\" name=\"Button\" value=\"Salvar\" >";
	echo '</td>
             <td align="center">&nbsp;</td>
             <td>&nbsp;</td>
         </tr></table>';
	...
	//
		...
		//$tmp_recd = " WHERE recf.nu_seq_feriado = '".$all_Rfuncao->rfunc_idrecurso[0]."' ORDER BY recf.dt_feriado ASC ";
		$tmp_recf = " ORDER BY recf.dt_feriado ASC ";
		$all_Recf = new request();
		$all_Recf->openRecferiado($tmp_recf);
		$contRecf = count($all_Recf->recf_id);
		
		$block_mudanca = new block();
		$block_mudanca ->heading("<a href=\"#\" onClick=\"verif('imgmuda','mosdemanda','muda','0');\" class=\"asem\"><strong style=color=\"blue\";font-size=\"23\"; font-weight: bold;><img id=\"imgmuda\" src=\"../themes/".THEME."/btn_soma.gif\" alt=\"\" border=\"0\"></strong></a> Feriados:  ".$mes_ativ );
	
      echo "<input type=\"hidden\" name=\"mosdemanda\" value=\"0\"  id=\"mosdemanda\" >";
		 
      echo "<table id='adm' style=\"display:'none'\"><tr><td>";
      echo '</td></tr></table>';
      echo "<table id='muda' cellspacing='0' width=\"100%\" border='0' cellpadding='2' cols='4'  bgcolor='#C4D3DB' style=\"display:'none'\">
				<tr align='left'>
					<th ></th>
					<th width='30%'>".$strings["date"]." </th>
					<th width='25%'>".$strings["descricao"]."</th>
					<th ></th>
				</tr>";
				
		$cont_som = 0 ;		
		for($k=0; $k < $contRecf;$k++ ){
			...
			echo "<tr class='$class_som' onmouseover=\"this.style.backgroundColor='".$block1->highlightOn."'\" onmouseout=\"this.style.backgroundColor='".$highlightOff."'\">
						<td width='15%' ></td>
						<td width='30%'>".$block1->buildLink("../administration/recursoferiado.php?did=".$all_Recf->recf_id[$k]."&type=dispensaEdit",invert_data($all_Recf->recf_data[$k]),"in","Editar Feriado.")."</td> 
						<td width='25%'>&nbsp;&nbsp;&nbsp;".$all_Recf->recf_descricao[$k]."</td>
