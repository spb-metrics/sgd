...
//if ((!$profilSession == 6) && (!$profilSession == 10) && (!$profilSession == 0)) {
//	headerFunction('../general/permissiondenied.php?' . session_name() . '=' . session_id());
//	exit;
//}

if ((!$select_de == 0) && (!$select_para == 0)) {
	if ($select_para != $select_de) {
		//echo "id e ".$select_de."-- para".$select_para;

		$tmp = '';

		//
		echo "<script>alert('Alterado com Sucesso!');window.location.href='serv_demandas.php?';</script>";
		exit;

	}
}

//INI CADASTRA USUÁRIO

if ($nome != "" && $AtLoc != "" && $idusu != "" && $senha != "" && $email != "" && $telefone != "") {
	//echo "perfil:".$AtPel;
	switch ($AtPel) {
		case 'sgd_coordenador_relacionamento' :
			//echo "<br>i igual a 6";
			$controle_perfil = "6";
			break;
		case 'sgd_gestor' :
		...
		case 'sgd_coor_relat' :
			...
			$controle_perfil = "9";
			break;
		case 'sgd_coor_fabric' :
			// echo "<br>i igual a 10";
			$controle_perfil = "10";
			break;
		case 'sgd_gerente_projeto' :
			// echo "<br>i igual a 1";
			$controle_perfil = "1";
			break;
		case 'sgd_analista_relat' :
			// echo "<br>i igual a 11";
			$controle_perfil = "11";
			break;
		case 'sgd_usuario' :
			// echo "<br>i igual a 2";
	...
	//---Ini Captura ultimo usuário
	last_id($tableCollab["members"]);
	$usuario_last = $lastId[0];
	//---Fim Captura ultimo usuário

	//---Ini Captura grupo Selecionado
	$tmp_grupo = " WHERE SG.NO_GRUPO = '" . $AtPel . "' ";
	$sel_Grupo = new request();
	$sel_Grupo->openGruposistema($tmp_grupo);
	$contGrupo = count($sel_Grupo->sg_id);
	//---Fim Captura grupo Selecionado

	$tmp_sistema = " INSERT INTO " . MYDATABASE . ".".$tableCollab["s_usuario_grupo"]." (NU_SEQ_GRUPO,NU_SEQ_USUARIO,ST_ATIVO) VALUES ('" . $sel_Grupo->sg_id[0] . "','" . $usuario_last . "', 'S')";
	connectSql("$tmp_sistema");

	echo "<script>alert('Cadastrado com Sucesso!');window.location.href='regist.php?';</script>";
	exit;
...
//FIM CADASTRA USUÁRIO

include ('../themes/' . THEME . '/header.php');
?>
<script language="JavaScript">

function verificaAddusuario(){

var nome = document.addsupport.nome.value;
var idusu = document.addsupport.idusu.value;
var senha = document.addsupport.senha.value;
var email = document.addsupport.email.value;
var AtPel = document.addsupport.AtPel.value;
var AtLoc = document.addsupport.AtLoc.value;
var telefone = document.addsupport.telefone.value;
	...
	if(AtLoc==""){
	alert("Selecione a Lotação!");
	return false	
	}else
	
	if(telefone==""){
	alert("Selecione o Telefone!");
	return false	
	}else{
...
echo "<form accept-charset='UNKNOWN' onSubmit='return verificaAddusuario();' method='POST' name='addsupport' action='../administration/addusuario.php?" . session_name() . "=" . session_id() . "&action=true' enctype='multipart/form-data' >";

$blockPage = new block();
$blockPage->openBreadcrumbs();
$blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/admin.php?", $strings["administration"], in));
$blockPage->itemBreadcrumbs($blockPage->buildLink("services.php?", $strings["service_management"], in));
if (!$change == "") {
	$blockPage->itemBreadcrumbs($blockPage->buildLink("serv_demandas.php?", $strings["demandas_management"], in));
}
$blockPage->itemBreadcrumbs(" Opções de " . $strings["demandas_management"]);
...
//
$tmp_organization = "   ";
$sel_Organization = new request();
$sel_Organization->openOrganizations($tmp_Organization);
$contOrganization = count($sel_Organization->org_id);
//
...
if ($change == "") {
	...
	$s_heading = "Administração de Usuário"; //$strings["service_management"];
	$s_contentTitle = "Cadastro de Usuário"; //$strings["demandas_management"];
}
//

$blockAdd->heading($s_heading);
$blockAdd->openContent();
$blockAdd->contentTitle($s_contentTitle, "4");

// INI USUARIO

$insNome = $blockAdd->insInput("nome", "nome", "70", "", "", "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Nome:", "40%", "");
$blockAdd->cellRow($insNome, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insId = $blockAdd->insInput("idusu", "idusu", "40", "", "", "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Usuário:", "40%", "");
$blockAdd->cellRow($insId, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insSenha = $blockAdd->insInput("senha", "senha", "40", "", "", "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Senha:", "40%", "");
$blockAdd->cellRow($insSenha, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insEmail = $blockAdd->insInput("email", "email", "40", "", "", "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Email:", "40%", "");
$blockAdd->cellRow($insEmail, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$option_perfil = array (
	0 => "0",
	1 => "<strong>Selecione Perfil</strong>"
);

$insPerfil = $blockAdd->insSelect($sel_Grupo->sg_nome, $sel_Grupo->sg_descricao, "", "AtPel", "AtPel", "", $option_perfil, "200", "16", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Perfil:", "40%", "");
$blockAdd->cellRow($insPerfil, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$option_unidade = array (
	0 => $idlotacao,
	1 => "<strong>Selecione Unidade</strong>"
);
$insUnidade = $blockAdd->insSelect($sel_Organization->org_id, $sel_Organization->org_name, $idlotacao, "AtLoc", "AtLoc", "", $option_unidade, "200", "16", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Unidade:", "40%", "");
$blockAdd->cellRow($insUnidade, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insTelefone = $blockAdd->insInput("telefone", "telefone", "20", "", "", "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Telefone:", "40%", "");
$blockAdd->cellRow($insTelefone, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insButton = $blockAdd->insButton("1","Submit","Submit","", "", "Confirmar", "", "", "");
$blockAdd->openContent();
$blockAdd->contentRow("",$insButton, "center");

// FIM USUARIO

$blockAdd->closeContent();
