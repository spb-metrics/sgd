...
$localizacaoatualizada = $_REQUEST['AtLoc'];
$telatualizado = $_REQUEST['tel2'];
$sigla = $_REQUEST['sisgs'];
$id_gestor = $_REQUEST['id_usu'];
$novo = $_REQUEST['novo'];
$sis = $_REQUEST['sis'];

//ADCIONAR SISTEMA DA BASE DE DADOS AO GESTOR
if ($id_gestor != "" && $sigla != "") {

	$tmp_sistema = " WHERE CO_SISTEMA = '" . strtoupper(trim($sigla)) . "' ";
	$sel_Sistema = new request();
	$sel_Sistema->openSosistema($tmp_sistema);
	$contSistema = count($sel_Sistema->sis_id);
	$id_sistema = $sel_Sistema->sis_id[0];

	if (($id_sistema != "") || ($id_sistema != 0) || ($id_sistema != '0')) {

		$tmp_sistema = " UPDATE SGD_FNDE.S_SISTEMA_GESTOR SET NU_SEQ_GESTOR = '" . $id_gestor . "' WHERE NU_SEQ_SISTEMA = '" . $id_sistema . "'";
		connectSql("$tmp_sistema");

		$tmp_gestor = " UPDATE SGD_FNDE.MEMBERS SET PHONE_WORK = '" . $telatualizado . "', ORGANIZATION = '" . $localizacaoatualizada . "' WHERE ID = '" . $id_gestor . "'";
		connectSql("$tmp_gestor");

		header('Location:Formgest.php?gsst=' . $id_usu . '');
	}
}

include_once "../themes/" . THEME . "/header_window.php";

$id_usu = $_REQUEST['id_usu'];

//
$tmp_sistema = " WHERE nu_seq_gestor_aplicacao != '" . $id_usu . "' ";
$sel_Sistema = new request();
$sel_Sistema->openSosistema($tmp_sistema);
$contSistema = count($sel_Sistema->sis_id);

//
$tmp_member = " WHERE mem.id = '" . $id_usu . "'  ";
$sel_Member = new request();
$sel_Member->openMembers($tmp_member);
$contMember = count($sel_Member->mem_id);

for ($j = 0; $j < $contMember; $j++) {

	$nome_usu = $sel_Member->mem_name[$j];
	$id_us = $sel_Member->mem_id[$j];
	$email = $sel_Member->mem_email_home[$j];
	$idlotacao = $sel_Member->mem_organization[$j];
	$lotacao = $sel_Member->mem_org_name[$j];
	$tel = $sel_Member->mem_phone_work[$j];
...
$insNome = $block1->insInput("nome", "nome", "70", "", $nome_usu, "", "17", "disabled");
$insidusu = $block1->insInput("id_usu", "id_usu", "50", "", $id_us, "", "17", "disabled", "2");

$block1->openForm("../administration/FormAlt.php", "formAlt");

$block1->openRow("on");
$block1->cellRow($insidusu, "1%", "", "", "0");
$block1->cellRow("Nome:", "15%", "", "", "0", "0");
$block1->cellRow($insNome, "83%", "", "", "2", "1");
$block1->cellRow("&nbsp;", "1%", "", "", "0");
$block1->closeRow();

$insEmail = $block1->insInput("emailgs", "emailgs", "70", "", $email, "", "17", "disabled");

$block1->openRow("on");
$block1->cellRow("", "1%", "", "", "0");
$block1->cellRow("Email:", "15%", "", "", "0");
$block1->cellRow($insEmail, "83%", "", "", "2");
$block1->cellRow("", "1%", "", "", "1");
$block1->closeRow();

$insLotacao = $block1->insInput("lotacaogs", "lotacaogs", "17", "", $lotacao, "", "17", "disabled");
$option_unidade = array (
	0 => $idlotacao,
	1 => "<strong>" . $lotacao . "</strong>"
);
$insLocalizacao = $block1->insSelect($sel_Organization->org_id, $sel_Organization->org_name, $idlotacao, "AtLoc", "AtLoc", "", $option_unidade, "", "17", "");

$block1->openRow("on");
$block1->cellRow("", "1%", "", "", "0");
$block1->cellRow("Lotação:", "10%", "", "", "0");
$block1->cellRow($insLotacao, "40%", "", "", "0");
$block1->cellRow("Localização: " . $insLocalizacao, "49%", "", "", "2");
$block1->closeRow();

$insTelefone = $block1->insInput("tel", "tel", "17", "", $tel, "", "17", "disabled");
$insTelefoneatual = $block1->insInput("tel2", "tel2", "17", "", $tel, "", "17", "");

$block1->openRow("on");
$block1->cellRow("", "1%", "", "", "0");
$block1->cellRow("Telefone:", "10%", "", "", "0");
$block1->cellRow($insTelefone, "40%", "", "", "0", "1");
$block1->cellRow("Telefone Atualizado:" . $insTelefoneatual, "49%", "", "", "", "1");
$block1->cellRow("", "40%", "", "", "");
$block1->closeRow();

$block1->openRow("on");
$block1->cellRow("&nbsp;", "100%", "", "", "5");
$block1->closeRow();

$block1->closeContent();

//INI SISTEMA PARA O GESTOR 
...
$block1->openContent();
$block1->contentTitle("&nbsp;Relacionar Sistema", "5");

$option_sistema = array (
	0 => "0",
	1 => "<strong>Selecione Sistema</strong>"
);

$insSistema = $block1->insSelect($sel_Sistema->sis_nome, $sel_Sistema->sis_nome, $index, "sisgs", "sisgs", "", $option_sistema, "", "17", "onchange=\"Dados('sis','sisformalt.php','sis',this.value,'sis','Selecione um Sistema','1');\"");

$block1->openRow("on");
$block1->cellRow("", "1%", "", "", "0");
$block1->cellRow("Sigla do Sistema:", "33%", "", "", "0");
$block1->cellRow($insSistema, "37%", "", "", "0");
$block1->cellRow("&nbsp;", "29%", "", "", "");
$block1->cellRow("", "1%", "", "", "");
$block1->closeRow();

$option_sistema2 = array (
	0 => "opcoes",
	1 => "<strong>Selecione Sistema</strong>"
);
$insSistema2 = $block1->insInput("sis", "sis", "45", "", "", "", "17", "");

$block1->openRow("on");
$block1->cellRow("", "1%", "", "", "");
$block1->cellRow("Nome do Sistema:", "33%", "", "", "");
$block1->cellRow($insSistema2, "37%", "", "", "");
$block1->cellRow("", "29%", "", "", "");
$block1->cellRow("", "1%", "", "", "");
$block1->closeRow();
...
$block1->cellRow("&nbsp;", "100%", "", "", "5");
$block1->closeRow();
//FIM SISTEMA PARA O GESTOR 	

$insButton = $block1->insButton("1", "submit", "submit", "", "", "Salvar", "", "", "");

$block1->openRow("on");
$block1->cellRow($insButton."&nbsp;&nbsp;<a href='Formgest.php?gsst=" . $id_usu . "'>| Voltar |</a> ", "80%", "", "", "5", "2");
$block1->closeRow();
