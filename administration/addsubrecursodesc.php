...
include_once('../includes/library.php');
 ...
 if ($project != "") {
   ...
   //INI MOSTRA FUNCAO
   if($idrec!=""){
   $tmp_func = " where func.nu_seq_funcao_sgd = ".$idrec." ";
   $all_Funcao = new  request();
   $all_Funcao->openFuncao($tmp_func);
   $contFunc = count($all_Funcao->func_id);
   }
   //FIM MOSTRA FUNCAO

   //INI MOSTRA SUBFUNCAO
   if($idrec!=""){
     $tmp_sfunc = " WHERE SUBF.NU_SEQ_FUNCAO_SGD = ".$all_Funcao->func_id[0]." ";
     $all_Sfuncao = new request();
     $all_Sfuncao->openSubfunc($tmp_sfunc);
     $contSfunc = count($all_Sfuncao->sfunc_id);
   //echo "<br>mostra sub".$contSfunc."<br>";
   }

   //FIM MOSTRA SUBFUNCAO

  //INI GERA NOVA SUBFUNCAO
  if($contSfunc < 1 && $idrec!=""){
    $tmpquery1 = "INSERT INTO ".MYDATABASE.".S_SUB_FUNCAO_SGD (no_sub_funcao_sgd,nu_seq_funcao_sgd) VALUES('".$all_Funcao->func_descricao[0]."','$idrec')";
    connectSql($tmpquery1);
    headerFunction('../administration/addsubrecursodesc.php?idrec='.$idrec.'&'.session_name().'='.session_id());
    exit;

  }
  //FIM GERA NOVA SUBFUNCAO

  //INI MOSTRA RECURSO USUARIO

  $tmp_Rfunc = " AND RU.NU_SEQ_USUARIO IS NULL ORDER BY NOME ";
  ...
  //INI MOSTRA  USUARIO NO RECURSO
  if($idrec!=""){
    $tmp_Rrfunc = " AND RU.NU_SEQ_USUARIO=id AND RU.nu_seq_sub_funcao_sgd = '".$all_Sfuncao->sfunc_id[0]."'ORDER BY NOME";
    $all_Rrfuncao = new  request();
    $all_Rrfuncao->openRecurso_usuario($tmp_Rrfunc);
    $contRrfunc = count($all_Rrfuncao->rfunc_id);
  //echo $contRrfunc ;
  }
  //FIM MOSTRA USUARIO NO RECURSO 

  if($action == "cbx"){
  
	 if($control==1){
		   for($y=0;$y < count($selectuser);$y++){
		   $tmpquery1 = "INSERT INTO SGD_FNDE.S_RECURSO_USUARIO (nu_seq_sub_funcao_sgd,nu_seq_usuario,ds_recurso_usuario) VALUES('$subfuncao','$selectuser[$y]','Sistemas')";
		   connectSql($tmpquery1);
		   } 
	 }
			//exit;
			if($control==2){
				  for($y=0;$y < count($selectrec);$y++){
				  $tmpquery1 = " DELETE FROM SGD_FNDE.S_RECURSO_USUARIO WHERE nu_seq_usuario = '$selectrec[$y]' ";
				  connectSql($tmpquery1);
				  }
			}
		
		
	headerFunction('../administration/addsubrecursodesc.php?idrec='.$idrecs.'&'.session_name().'='.session_id());
  }
...
if ($action == "add") {
	...
	$mes = convertData($mes);
	 ...
	 $tmpquery1 = "INSERT INTO SGD_FNDE.S_SUB_FUNCAO_SGD (no_sub_funcao_sgd,nu_seq_funcao_sgd) VALUES('$tit','$user')";
	connectSql($tmpquery1);
			
	headerFunction("recurso.php?&".session_name()."=".session_id());
	exit;
}
  ...
  $ll = $projectDetail->pro_sistema[0];
	if(!$ll==""){
     ...
     $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/recurso.php?","Função",in));
     $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/recursousuario.php?fc=".$all_Sfuncao->sfunc_id[0],$all_Sfuncao->sfunc_nofuncao[0],in));
     $blockPage->itemBreadcrumbs($strings["admin_intro"]);
     $blockPage->closeBreadcrumbs();


  echo "<br><form  method='POST' action='addsubrecursodesc.php?action=cbx&".session_name()."=".session_id()."&' name='addsup' enctype='multipart/form-data'  >";
	
	$block1 = new block();
	$block1->heading($strings["user_management_recurso"]." - ".$all_Sfuncao->sfunc_nofuncao[0]);
	
  echo '<table width="90%" border="0" cellspacing="0" cellpadding="0" class="content">';

       $block1->contentSub("Usuários da Função","4");  

  echo '<tr class="odd">
            <td width="42%" align="center"><strong>Usuários</strong></td>
            <td width="8%">&nbsp;</td>
            <td width="8%">&nbsp;</td>';
        echo" <td width=\"42%\" align=\"center\"><strong>".$all_Sfuncao->sfunc_nofuncao[0]."</strong></td>
       </tr>";

  echo '<tr class="odd">
            <td rowspan="2" align="center">';
	         echo "<select name=\"selectuser[]\" size=\"6\" multiple style=\"width:250px; height:150px; font-size:9px   \" onChange=\"TrocaList(1,'document.addsupport.control.value')\">";
                    for($z=0 ; $z < $contRfunc; $z++){
	                echo "<option value=\"".$all_Rfuncao->rfunc_id[$z]."\">".$all_Rfuncao->rfunc_nome[$z]."</option>";
                    }
                echo "</select>";
      echo '</td>
            <td height="10" colspan="2" align="center" valign="bottom">';
	         echo "<input type=\"submit\" name=\"Button\" value=\"&gt;&gt;\" title=\"Adicionar\"><BR>";
      echo "</td> ";
      echo '<td rowspan="2" align="center">';
	         echo "<select name=\"selectrec[]\" size=\"6\" multiple style=\"width:250px;height:150px; font-size:9px  \" onChange=\"TrocaList(2,'document.addsupport.control.value')\">";
                     for($j=0 ; $j < $contRrfunc; $j++){
		          echo "<option value=\"".$all_Rrfuncao->rfunc_id[$j]."\">".$all_Rrfuncao->rfunc_nome[$j]."</option>";
		       }  
		  echo "</select>";
	echo '</td>
       </tr>
  ...
  <tr class="odd">
	...
	echo "<input type=\"hidden\" name=\"subfuncao\" value=\"".$all_Sfuncao->sfunc_id[0]."\">";
	echo "<input type=\"hidden\" name=\"idrecs\" value=\"".$idrec."\">";
	echo "<br><input type=\"submit\" name=\"Button\" value=\"&lt;&lt;\" title=\"Remover\" >";
