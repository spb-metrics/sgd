...
if ($change == "edit" && $action=="true") {
//UPDATE UNIDADE

	if ($nome != "" && $sigla != "" ) {
	
	$tmp_unidade = " UPDATE " . MYDATABASE . ".".$tableCollab["organizations"]." SET name='" .strtoupper($sigla) . "',comments='" . strtoupper($nome) . "' WHERE  id='".$id."' ";
	connectSql("$tmp_unidade");
	
	
	echo "<script>alert('Alterado com Sucesso!');window.location.href='viewunidade.php?';</script>";
		exit;
	}

//UPDATE CADASTRA UNIDADE
}
...
if ($nome != "" && $sigla != "" ) {
	...
	//---Ini Verifica Unidade
	$tmp_unidade = " WHERE org.name = '" .$sigla . "' OR org.comments = '".$nome."'  ";
	$sel_Unidade = new request();
	$sel_Unidade->openOrganizations($tmp_unidade);
	$contUnidade = count($sel_Unidade->org_id);
	//---Fim Verifica Unidade
	
if($contUnidade <= 0){
	$tmp_unidade = " INSERT INTO " . MYDATABASE . ".".$tableCollab["organizations"]." (name,comments,created) VALUES ('" .strtoupper($sigla) . "','" . strtoupper($nome) . "', '".date('Y-m-d')."')";
	connectSql("$tmp_unidade");

	echo "<script>alert('Cadastrado com Sucesso!');window.location.href='viewunidade.php?';</script>";
	exit;
	
}else{
echo "<script>alert('Unidade ja Existe!');window.history.back();</script>";
exit;	
}
...
var sigla = document.addsupport.sigla.value;
...
echo "<form accept-charset='UNKNOWN' onSubmit='return verificaAddusuario();' method='POST' name='addsupport' action='../administration/addunidade.php?change=".$change."&" . session_name() . "=" . session_id() . "&action=true' enctype='multipart/form-data' >";
...
if ($change == "") {
	...
	$s_heading = "Administração de Unidade"; 
	$s_contentTitle = "Cadastro de Unidade"; 
}else if($change == "edit"){
	
   $s_heading = "Administração de Unidade"; 
   $s_contentTitle = "Altera Unidade"; 
//
	//---Ini Verifica Unidade
	$tmp_unidade = " WHERE org.id = '" .$id . "'   ";
	$sel_Unidade = new request();
	$sel_Unidade->openOrganizations($tmp_unidade);
	$contUnidade = count($sel_Unidade->org_id);
	//---Fim Verifica Unidade
...
if ($change == "edit") {
	$nomeValue = $sel_Unidade->org_descricao[0];
	$siglaValue = $sel_Unidade->org_name[0];
	$idValue = $sel_Unidade->org_id[0];
	$confirmaSubmit = "Alterar";
}else{
    $nomeValue = "";
	$siglaValue = "";
	$confirmaSubmit = "Confirmar";	
}

$insNome = $blockAdd->insInput("nome", "nome", "70", "",$nomeValue, "", "17", "");
$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Nome da Unidade:", "40%", "");
$blockAdd->cellRow($insNome, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insSigla = $blockAdd->insInput("sigla", "sigla", "40", "", $siglaValue, "", "17", "");
$insid = $blockAdd->insInput("id", "id", "40", "",$idValue, "", "17", "","2");

$blockAdd->openRow("on");
$blockAdd->cellRow("", "10%", "");
$blockAdd->cellRow("Sigla:".$insid, "40%", "");
$blockAdd->cellRow($insSigla, "40%", "");
$blockAdd->cellRow("", "10%", "");
$blockAdd->closeRow();

$insButton = $blockAdd->insButton("1","Submit","Submit","", "",$confirmaSubmit, "", "", "");
$blockAdd->openContent();
