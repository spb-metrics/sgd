...
$User = $_REQUEST["uni"];    
$Tipo = $_REQUEST["tp"];   
$Unidade = $_REQUEST["unidad"]; 
if((!$Unidade=="") && ($Unidade > 0)){
$tmpq = " pro.organization = '".$Unidade."' AND  ";
}else{
$tmpq = " ";
}   
$tmp = "";
switch($Tipo){
case 'analista':
	 $tmp = " $tmpq pro.id_analista='".$User."' and pro.id_cordenacao='".$User."' ";
	 break;
case 'gerente':
	 $tmp = " $tmpq pro.id_ger_pro='".$User."' ";
	 break;
case 'gestor':
	 $tmp = " $tmpq pro.owner='".$User."' ";
	 break;

}

 		//1DEMANDA EM ABERTO
 		$tmpqueryy = " WHERE  $tmp and pro.status='3' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contAberto = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//2DEMANDA EM ANALISE
 		$tmpquery = " WHERE  ".$tmp." and pro.status='1' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpquery);
		$contAnalise = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//3DEMANDA VALIDO PELO RELACIONAMENTO
 		$tmpqueryy = " WHERE  $tmp and pro.status='4' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contVrel = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//4DEMANDA VALIDO PELA TI
 		$tmpqueryy = " WHERE  $tmp and pro.status='5' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contVti = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//5DEMANDA VALIDO PELO CLIENTE
 		$tmpqueryy = " WHERE  $tmp and pro.status='0' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contVc = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//6DEMANDA EM ANDAMENTO
 		$tmpqueryy = " WHERE  $tmp and pro.status='6' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contEand = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//7DEMANDA AGUARDANDO HOMOLOGAÇÃO
 		$tmpqueryy = " WHERE  $tmp and pro.status='7' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contAm = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//8DEMANDA EM HOMOLOGAÇÃO
 		$tmpqueryy = " WHERE  $tmp and pro.status='9' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contEm = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//9DEMANDA ACEITA PELO CLIENTE
 		$tmpqueryy = " WHERE  $tmp and pro.status='8' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contAc = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//10DEMANDA CONCLUIDA
 		$tmpqueryy = " WHERE  $tmp and pro.status='10' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contCon = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//11DEMANDA SUSPENSA
 		$tmpqueryy = " WHERE  $tmp and pro.status='2' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contSus = count($listProjects->pro_id);
 		unset($listProjects);
		//
		//12DEMANDA PENDENTE
 		$tmpqueryy = " WHERE  $tmp and pro.status='12' ";
		$listProjects = new request();
		$listProjects->openProjects($tmpqueryy);
		$contCan = count($listProjects->pro_id);
 		unset($listProjects);
		//
...
if(!$tmp=="") {                
   ...
   $xml .= "<gestores>\n";               
	  ...
	  $xml .= "<gestor>\n";     
      $xml .= "<codigo>".$contAberto."</codigo>\n";
	  $xml .= "<codigo2>".$contAnalise."</codigo2>\n"; 
	  $xml .= "<codigo3>".$contVrel."</codigo3>\n";
	  $xml .= "<codigo4>".$contVti."</codigo4>\n";  
	  $xml .= "<codigo5>".$contVc."</codigo5>\n";  
	  $xml .= "<codigo6>".$contEand."</codigo6>\n";  
	  $xml .= "<codigo7>".$contAm."</codigo7>\n";  
	  $xml .= "<codigo8>".$contEm."</codigo8>\n";
	  $xml .= "<codigo9>".$contAc."</codigo9>\n"; 
	  $xml .= "<codigo10>".$contCon."</codigo10>\n";
	  $xml .= "<codigo11>".$contSus."</codigo11>\n";
	  $xml .= "<codigo12>".$contCan."</codigo12>\n";                         
      $xml .= "</gestor>\n";    
