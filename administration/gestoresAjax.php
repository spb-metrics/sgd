...
//QUERY  
    ...
    $tmp_gestores = " AND SG.NO_GRUPO='sgd_gestor' AND SUO.ID='".$Unidade."' ";
		$sel_Gestores = new request();
		$sel_Gestores->openAplicacao($tmp_gestores);
		$contGestores = count($sel_Gestores->apl_id);
...
if($contGestores) {                
   ...
   for($i=0; $i< $contGestores ; $i++) {  
    
	  $codigo = $sel_Gestores->apl_id[$i];
	  $descricao = $sel_Gestores->apl_login[$i];
