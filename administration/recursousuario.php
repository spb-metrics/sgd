...
*/
  ...
  $tmp_Usu = " AND RU.nu_seq_sub_funcao_sgd='".$fc."'";
  $sel_Usu = new  request();
  ...
  $tmp_Subfunc = " WHERE SUBF.nu_seq_sub_funcao_sgd= '".$fc."' ";
  $Subfunc = new  request();
  ...
  //FIM MOSTRA SUBFUNCAO
    ...
    $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/recurso.php?",$strings["recurso"],in));
    ...
    $block1 = new block();
    $block1->heading($strings["recurso_management"]." - ".$Subfunc->sfunc_nofuncao[0]);
    $block1->openContent();
    $block1->arrayTitle($sortingFields = array(0=>"-",1=>"Nome:",2=>"Habilidades"),$widthFields = array(0=>"5%",1=>"55%",2=>"40%"));
    $block1->openTablelist();
      ...
      for($i= 0;$i < $contUsu;$i++){
        ...
        $block1->openRow("");
        $block1->cellRow("",'5%',"", '25'); 
        $block1->cellRow($blockPage->buildLink("recursousudesc.php?idusu=".$sel_Usu->rfunc_id[$i],"<strong>".$sel_Usu->rfunc_nome[$i]."</strong>",in),'40%',"", '25');
        $block1->cellRow("&nbsp;<span style=\"cursor:hand;\" title=\"".$sel_Usu->rfunc_habilidade[$i]."\">".substr($sel_Usu->rfunc_habilidade[$i],0,62)."</span>",'55%',"", '25');
        $block1->closeRow(); 
