...
//INI REMOVE LOG DE EMAIL
if(($rmlog=="ok") && ($rmid!="")){

if($rmdt!=""){
	$tmp = " WHERE an_date='".$rmdt."' ";
	}else{
	$tmp = " WHERE nu_seq_id='".$rmid."' ";
	}
	$controD = new request();
	$controD->openControle($tmp);
	$contControle = count($controD->sco_id);
	//$dataver = $controD->sco_data[0];
	
	//$tmpimpatar = " WHERE calr.id_calendar = '".$id."'" ;
	//$requestatar = new request();
	//$requestatar->openAtar("$tmpimpatar");
	//$countAtar = count($requestatar->atar_id);
	
	for($i=0; $i < $contControle; $i++ ){
	$dataver = $controD->sco_data[$i];
	chmod($controD->sco_documento[$i],0777);
	$arquivo = $controD->sco_documento[$i];
	...
	$tmpquery2 = "DELETE FROM ".MYDATABASE.".s_control_in ".$tmp."";
	connectSql("$tmpquery2");
	
	headerFunction("../administration/serv_autentica.php?mailData=".$dataver."&".session_name()."=".session_id());
	exit;

}
//FIM REMOVE LOG DE EMAIL


$tmp = " WHERE st_status = '2' ";

if(!$mailDetail==""){

$tmp .= " AND nu_seq_id='".$mailDetail."' ";

}

if(!$mailData==""){

$tmp .= " AND an_date='".$mailData."' ";

}

$tmp .= " ORDER BY nu_seq_id DESC";
$controD = new request();
$controD->openControle($tmp);
$contControle = count($controD->sco_id);
...
$block1->openContent();
$block1->contentTitle($strings["autentica_management"]);
if($borne1==""){
$borne1 = 0;
} 
if($mailDetail==false && $mailData==""){
	$conv = 0;
	$vald = intval($borne1+10);
	for($i=0; $i < $contControle; $i++){
					
				if($controD->sco_data[$i] != $controD->sco_data[$i-1]){
					if(($conv>=$borne1)&&($conv < $vald)){
							if (!($i%2)) {
							$class = "odd";
						...
						//echo "mostra".$controD->sco_id[$i];
						//$block1->contentRow("",$blockPage->buildLink("../users/listusers.php?",invert_data($controD->sco_data[$i])."-".$controD->sco_processo[$i],in));
							$block1->openRow();
							$block1->cellRow("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$blockPage->buildLink("../administration/serv_autentica.php?mailData=".$controD->sco_data[$i]."",invert_data($controD->sco_data[$i]),in),'80%');
							//$block1->contentRow("",$blockPage->buildLink("../administration/serv_autentica.php?mailData=".$controD->sco_data[$i]."",invert_data($controD->sco_data[$i]),in));
							$block1->cellRow($blockPage->buildLink("../administration/serv_autentica.php?rmdt=".$controD->sco_data[$i]."&rmlog=ok&rmid=".$controD->sco_id[$i]."","Remover",in),'20%');
							$block1->closeRow();
...
if(!$mailData==""){
	...
	//echo "mostra".$controD->sco_id[$i];
		...
		if(($i>=$borne1)&&($i<$vald)){
			...
			//$block1->contentRow("",$blockPage->buildLink("../administration/serv_autentica.php?mailDetail=".$controD->sco_id[$i]."",invert_data($controD->sco_data[$i])."-".$controD->sco_processo[$i],in));
			
			$block1->openRow();
			//$block1->cellRow("",'10%');
			$block1->cellRow("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$blockPage->buildLink("../administration/serv_autentica.php?mailDetail=".$controD->sco_id[$i]."",invert_data($controD->sco_data[$i])."- Numero:".$controD->sco_id[$i].", ".$controD->sco_processo[$i],in),'80%');
			$block1->cellRow($blockPage->buildLink("../administration/serv_autentica.php?rmlog=ok&rmid=".$controD->sco_id[$i]."","Remover",in),'20%');
			$block1->closeRow();
	     }
	++$conv;
	} //fim for;
$block1->recordsTotal = $conv ;	
$typeProjects = "mailData=".$mailData;

}

if(!$mailDetail=="" && $mailData==""){
	for($i = 0; $i < $contControle; $i++){
	//echo "mostra".$controD->sco_id[$i];
	//$block1->contentRow("",$blockPage->buildLink("../users/listusers.php?",invert_data($controD->sco_data[$i])."-".$controD->sco_processo[$i],in));
	$block1->contentRow("",$blockPage->buildLink("../administration/serv_autentica.php?mailDetail=".$controD->sco_id[$i]."",invert_data($controD->sco_data[$i]),in));
	$block1->contentRow("",$blockPage->buildLink("../administration/serv_autentica.php?mailDetail=".$controD->sco_id[$i]."","Numero :".$controD->sco_id[$i]."-SITUAÇÃO:".$controD->sco_processo[$i],in));
	$gera = new arquivo();
	$documento = $gera->abrir($controD->sco_documento[$i]);
	$block1->contentRow("",'<span style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:9px";>'.$documento.'</span>');
