...
if (!$profilSession == 6) 
{
 ...
 echo "<form name=\"InfoGestor\"  method=\"POST\" onSubmit=\"return popupform(this,'Formgest.php','545','490');\">";
 
        $block1 = new block();
	    $block1->heading($strings["user_management_recurso"]);
    
		$block1->openContent("90");
		$block1->contentTitle("Editar Gestor") ;
    
        //
        $tmp_gestor = "  AND SG.NO_GRUPO= 'sgd_gestor'  ";
		$sel_Gestor = new request();
		$sel_Gestor->openSousuario($tmp_gestor);
		$contGestor = count($sel_Gestor->so_id);
		//echo $contGestor ;
   
    $opções = array(0=>"opcoes",1=>"Gestor");
    $select_gestor =  $block1->insSelect($sel_Gestor->so_id,$sel_Gestor->so_nome,"","gsst","gsst","7",$opções,"","","class=\"odd\" Onchange =\"Dadosz(this.value); Dados4(this.value); Dados5(this.value); Dados6(this.value);\"");
    $array_teste = array(0=>"",1=>"oi",2=>"teste",3=>"foi");
    
    
	    $block1->openContent();
		$block1->openRow("on");
		$block1->cellRow("","10%");
		$block1->cellRow($select_gestor,"40%");
		$block1->cellRow("","10%");
		$block1->cellRow("","40%");
		$block1->closeRow();
      
	  $block1->openRow("on");
	  $block1->cellRow("","100%","","","4");
	  $block1->closeRow();
    ...
    //FIM MOSTRA SISTEMA
 		...
 		$opções_sistema = array(0=>"opcoes2",1=>"Sistema");
    $sel_sistema =  $block1->insSelect($sel_Sistema->sis_id,$sel_Sistema->sis_nome,"","sist","sist","6",$opções_sistema,"","","class=\"odd\" Onchange =\"Dados3(this.value);\"");
   
    //
    $tmp_gerente = "  AND SG.NO_GRUPO= 'sgd_gerente_projeto'  ";
		$sel_Gerente = new request();
		$sel_Gerente->openSousuario($tmp_gerente);
		$contGerente = count($sel_Gerente->so_id);
		//echo $contGerente ;
          
    $opções_gerente = array(0=>"opcoes4",1=>"Gerente de Projeto");
    $sel_gerente =  $block1->insSelect($sel_Gerente->so_id,$sel_Gerente->so_nome,"","grp","grp","2",$opções_gerente,"","","class=\"odd\" ");
     
	  $block1->openRow("on");
	  $block1->cellRow("","10%");
	  $block1->cellRow($sel_sistema,"40%");
	  $block1->cellRow("","10%");
      $block1->cellRow($sel_gerente,"40%");
	  $block1->closeRow();  
    ...
    //
    $tmp_org = "  order by org.id  ";
    $sel_Org = new request();
    $sel_Org->openOrganizations($tmp_org);
    $contOrg = count($sel_Org->org_id);
    //echo $contOrg ;
          	    	
    $opções_unidade = array(0=>"opcoes3",1=>"Unidade");
    $sel_unidade =  $block1->insSelect($sel_Org->org_id,$sel_Org->org_name,"","uni","uni","2",$opções_unidade,"","","class=\"odd\" onChange=\"Dados1(this.value);\" ");
 	
 
    //
    $tmp_Analista = "  AND SG.NO_GRUPO= 'sgd_analista_relat'  ";
		$sel_Analista = new request();
		$sel_Analista->openSousuario($tmp_Analista);
		$contAnalista = count($sel_Analista->so_id);
		//echo $contAnalista ;
  
    $opcoes_analista = array(0=>"opcoes5",1=>"Analista de Relacionamento");
    $sel_analista =  $block1->insSelect($sel_Analista->so_id,$sel_Analista->so_nome,"","anl","anl","2",$opcoes_analista,"","","class=\"odd\" onChange=\"Dados7(this.value);\" ");
  
	  $block1->openRow("on");
	  $block1->cellRow("","10%");
	  $block1->cellRow($sel_unidade,"40%");
	  $block1->cellRow("","10%");
    $block1->cellRow($sel_analista,"40%");
	  $block1->closeRow();
		...
		$block1->openRow("on","off");
		$block1->cellRow($block1->insButton('1','editar','editar','','','Editar','','',''),"100%","","","4","2");
		$block1->closeRow();
