...
//headerFunction("../administration/horames?".session_name()."=".session_id());
...
$titlePage = $strings["welcome"]." $nameSession  ";
...
if ($type == "calendEdit") {
    if ($action == "update") 
    {
        ...
        $tmpquery = " UPDATE ".MYDATABASE.".S_RECURSO_PROJETO SET DT_RECURSO_PROJETO='".data_invert($dateStart)."', HR_RECURSO_PROJETO='$hora' WHERE NU_SEQ_REC_PROJ = '$idrecpro' ";
        connectSql("$tmpquery");
        //exit;
		headerFunction("../administration/horames?id=".$num."&dateEnreg=".$_SESSION['id_pro']."&dateCalend=".$dateCalend."&type=calendDetail&msg=update&".session_name()."=".session_id());
    ...
    if ($action == "add") 
    {
        ...
        else 
        {
		    ...
		    $tmpquery = " INSERT INTO ".MYDATABASE.".S_RECURSO_PROJETO (nu_seq_recurso_usuario, nu_seq_projeto,dt_recurso_projeto, hr_recurso_projeto) VALUES('$idrecurso','$num','".data_invert($dateStart)."','$hora') "; 
			
			connectSql("$tmpquery");
			
			$tmpquery = " ".MYDATABASE.".S_RECURSO_TEMPO ";
            last_id($tmpquery);
		    ...
		    echo '<script>window.location.href="../administration/horames?id='.$_SESSION['id_pro'].'&dateEnreg='.$num.'&type=calendDetail&dateCalend='.$dateCalend.'";</script>';
		}
    }
}
...
if ($type == "calendDetail") 
{
    ...
    if ($comptDetailCalendar == "0") 
    {
      headerFunction("../administration/horames.php?".session_name()."=".session_id());
    }
}
...
if ($type == "calendEdit") 
{
    ...
    $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/horames.php?type=monthPreview",$strings["recurso"],in));
    $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/horames.php?type=monthPreview&dateCalend=$dateCalend","$monthName $year",in));
  	$blockPage->itemBreadcrumbs("$dayName $day $monthName $year");

    if ($id != "") 
    {
        $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/horames.php?type=calendDetail&dateCalend=$dateCalend&dateEnreg=$dateEnreg",$detailCalendar->cal_shortname[0],in));
    ...
    if ($id != "") 
    {
        $block1->openForm("../administration/horames.php?".session_name()."=".session_id()."&dateEnreg=$dateEnreg&dateCalend=$dateCalend&type=$type&action=update#".$block1->form."Anchor");
    } 
    else 
    {
        $block1->openForm("../administration/horames.php?".session_name()."=".session_id()."&dateEnreg=$dateEnreg&dateCalend=$dateCalend&type=$type&action=add#".$block1->form."Anchor");
    }
    ...
    echo "
		...
		<tr class='odd'>
            <td valign='top' width='50'  class='leftvalue'>".$strings["hora_atividade"]." :</td>
            <td width='50'>
			<input size='24' style='width: 20px;' maxlength='2' type='text' name='hora' value='".$detailCalendar->recpro_hora[0]."'>
			</td>
        </tr>";
...
if ($type == "monthPreview") {
    ...
    $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/addatividade.php?id=".$_SESSION['id_pro']."",$strings["nova_atividade"],in));
	$blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/horames.php?type=monthPreview",$strings["recurso"],in));
    $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/horames.php?type=monthPreview&dateCalend=$dateCalend","$monthName $year",in));
    $blockPage->itemBreadcrumbs("$dayName $day $monthName $year");
    ...
    $block1->form = "calend";
    $block1->openForm("../administration/horames.php?".session_name()."=".session_id()."#".$block1->form."Anchor");
    ...
    $block1->closeForm();
		...
		for($k=0; $k < $comptListCalendarScan1;$k++ ){
			...
			echo "<tr class='$class_som' onmouseover=\"this.style.backgroundColor='".$block1->highlightOn."'\" onmouseout=\"this.style.backgroundColor='".$highlightOff."'\">
						...
						<td width='30%'>".$block_mudanca->buildLink("../administration/horames.php?id=".$listCalendarScan1->recpro_projeto[$k]."&dateEnreg=".$listCalendarScan1->recpro_id[$k]."&type=calendDetail&dateCalend=".$listCalendarScan1->recpro_data[$k]."",$selAtv1->rec_descricao[0],"in","Visualizar atividade")."</td>
		...
		$total_atividade  = $total_atividade + $listCalendarScan1->recpro_hora[$k];
		 ...
		 echo "</table>";
		 echo "<table width='100%'><tr><td align='left'>".$block1->buildLink("../administration/horames.php?id=$id&dateEnreg=$dateEnreg&type=calendDetail&dateCalend=".anteriorct($mes_so,$ano_so),"Anterior","in","Mostra Aterior atividades")."&nbsp;|&nbsp; ".$block1->buildLink("../administration/horames.php?id=$id&dateEnreg=$dateEnreg&type=calendDetail&dateCalend=".date('Y-m-d'),"Hoje","in","Mostra atividades de hoje")."&nbsp;|&nbsp;".$block1->buildLink("../administration/horames.php?id=$id&dateEnreg=$dateEnreg&type=calendDetail&dateCalend=".proximoct($mes_so,$ano_so),"Proximo","in","Mostra proxima atividades")."</td><td align='right'>Total Horas : $total_atividade</td></tr></table>";
   ...
   if ($profilSession == 6 || $profilSession == 7 ||  $profilSession == 1 )
    {
        ...
        if($listCalendarScan->recpro_data[0] >= $data_atual){
		$block1->paletteScript(0,"remove","../administration/deleterecurso.php?id=$dateEnreg","true,true,true",$strings["delete"]);
        }
		$block1->paletteScript(1,"edit","../administration/horames.php?id=$dateEnreg&type=calendEdit&dateCalend=$dateCalend","true,true,true",$strings["edit"]);
    }
    
    $block1->paletteScript(2,"export","../administration/addocumento.php?id=$dateEnreg&proje=".$detailCalendar->cal_projects[0]."","true,true,true",$strings["export"]);
