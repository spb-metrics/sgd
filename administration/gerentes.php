...
if($id_gerente == '0' || $id_gerente == 0 || $id_gerente !=""){
	
	$tmp_gerentes = " AND SG.NO_GRUPO='sgd_gerente_projeto' ";

 }else{
 
 $tmp_gerentes = " AND SG.NO_GRUPO='sgd_gerente_projeto' AND SU.ID=' ".$id_gerente." '";
    ...
    //INI MOSTRA GERENTES
    $sel_Gerentes = new request();
		$sel_Gerentes-> openSousuario($tmp_gerentes);
		$contGerentes = count($sel_Gerentes->so_id);
		//echo $contSistema ;
    //FIM MOSTRA GERENTES
...
if($contGerentes) {                
   ...
   for($i=0; $i < $contGerentes ; $i++) {  
     // OCIExecute($result2);
	  $codigo = $sel_Gerentes->so_id[$i];
	  $descricao = $sel_Gerentes->so_login[$i];
