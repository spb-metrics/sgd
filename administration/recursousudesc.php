...
*/
 ...
 include_once('../includes/library.php');
   ...
   if($action == "cbx"){
      ...
      $tmpq1 = " UPDATE sgd_fnde.s_recurso_usuario SET ds_recurso_usuario='".rtrim($textarea)."' WHERE nu_seq_usuario='".$idusu."'";
      connectSql("$tmpq1");
			
      headerFunction('../administration/recursousudesc.php?idusu='.$idusu.'&'.session_name().'='.session_id());
      exit;
   }
  ...
  if($idusu!=""){
  $tmp_Rfunc = " AND RU.NU_SEQ_USUARIO ='".$idusu."' ";
  ...
  $blockPage->itemBreadcrumbs($blockPage->buildLink("../administration/recursousuario.php?fc=".$all_Sfuncao->sfunc_id[0],"<strong>".$all_Sfuncao->sfunc_nofuncao[0]."</strong>",in));
  $blockPage->itemBreadcrumbs($strings["admin_intro"]);
  $blockPage->closeBreadcrumbs();

  
  echo "<br><form  method='POST' action='recursousudesc.php?idusu=".$idusu."&action=cbx&".session_name()."=".session_id()."&' name='addsup' enctype='multipart/form-data'  >";
  ...
  echo '<table width="99%" border="0" cellspacing="0" cellpadding="0"  class="content">';
       ...
       $block1->contentSub("Detalhes do Recurso","4");
 

  echo '<tr class="odd">
            <td width="8%" height="25" align="right">Usuário: </td>';
      echo "<td width=\"8%\">&nbsp;</td>";
      echo "<td width=\"42%\">".$all_Rfuncao->rfunc_nome[0]."</td>";
      echo " <td width=\"42%\" ></td>";
  echo '</tr>
         <tr class="odd">
	      <td width="8%" align="right" height="20">Função: </td>';
	echo '<td height="25"  align="center" valign="bottom"></td>';
	echo '<td rowspan="2"colspan="2" >';
	
	echo "<select name=\"selectrec\"  style=\"width=300; \">";
              $opt =" ";
		  for($j=0 ; $j < $contFunc; $j++){
		     $opt .=  "<option value=\"".$all_Funcao->func_id[$j]."\"  ";
		     if($all_Funcao->func_id[$j]==$all_Sfuncao->sfunc_idfuncao[0]){ $opt .= "selected";}
		    $opt .= ">".$all_Funcao->func_descricao[$j]."</option>";
		  }
	echo $opt ;  
	...
	echo '</td>
         ...
         <tr class="odd"><td  align="right" valign="top">&nbsp;Habilidades: </td>
             <td align="center">';
	...
	echo "<td height=\"50\" valign=\"top\">
                 <textarea name=\"textarea\" style=\"width=300; height=50 \" cols=\"45\" rows=\"6\" wrap=\"VIRTUAL\">".$all_Rfuncao->rfunc_habilidade[0]."</textarea>
             </td>";
	echo "<td align=\"center\">&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"recursodispensa.php?idusu=".$idusu."#\"><strong>Dispensas</strong></a>
             </td></tr>";
