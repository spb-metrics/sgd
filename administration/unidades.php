...
//RECEBE PARÃMETRO                     
$id_gestor = $_REQUEST["id"];    
    
if($id_gestor == '0' || $id_gestor == 0 || $id_gestor !=""){
 ...
 $tmp_organizations = " LEFT OUTER JOIN MEMBERS MEM ON MEM.ORGANIZATION=org.ID 
WHERE MEM.ID='".$id_gestor."'
GROUP BY org.id ";

}

    //INI MOSTRA UNIDADES
 		$sel_Organizations = new request();
		$sel_Organizations->openOrganizations($tmp_organizations);
		$contOrganizations = count($sel_Organizations->org_id);
	  //FIM MOSTRA UNIDADES

if($contOrganizations) {                
   //XML
   $xml  = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n";
   $xml .= "<unidades>\n";               
   
   for($i=0; $i < $contOrganizations ; $i++) {  
    
	  $codigo = $sel_Organizations->org_id[$i];
	  $descricao = $sel_Organizations->org_name[$i];
