...
//$con = OCILogon(MYLOGIN,MYPASSWORD,MYSERVER) or die($strings["error_server"]);
	 $sql = "SELECT DISTINCT(SA.sg_aplicacao) NOME, SA.NO_APLICACAO NAME
		...
		LEFT OUTER JOIN SGD_FNDE.PROJECTS SGDP ON SGDP.ID_SISTEMA = SA.nu_seq_aplicacao
		WHERE SGDP.ID_ANALISTA = '".$id_usu."'
		UNION ALL SELECT DISTINCT (SGDS.CO_SISTEMA) NOME, SGDS.NO_SISTEMA NAME FROM SGD_FNDE.SISTEMAS SGDS 
		LEFT OUTER JOIN ".MYDATABASE.".PROJECTS SGDP ON SGDP.ID_SISTEMA = SGDS.ID_SISTEMA
                WHERE SGDP.ID_ANALISTA = '".$id_usu."' ORDER BY NOME"; 
