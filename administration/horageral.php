...
*/
  ...
  //INI MOSTRA FUNCAO
  $tmp_hora = " ORDER BY reh.dt_periodo ";
  $all_Hora = new  request();
  $all_Hora->openRecursohora($tmp_hora);
  $contHora = count($all_Hora->reh_id);
  ...
  //FIM MOSTRA FUNCAO
   ...
   $blockPage->itemBreadcrumbs("Planejamento de Horas");
   ...
   $blockPage->closeBreadcrumbs();
	...
	$blockPage->messagebox($msgLabel);
       ...
       $block1 = new block();
       $block1->heading($strings["recurso_management"]." - Planejamento de Horas");

 echo "<a href='horasmes.php?id=&type=calendEdit&dateCalend=".date('Y-m-d')."'><img  id=\"admm\" src=\"../themes/".THEME."/btn_add_norm.gif\" alt=\"Nova Função\" border=\"0\" onmouseover=\"trocaImg('btn_add_over.gif','admm');\" onmouseout=\"trocaImg('btn_add_norm.gif','admm');\" style=\"display:''\"></a>";

       $block1->openContent();
       $block1->arrayTitle($sortingFields = array(0=>"-",1=>"Mês:",2=>"Horas"),$widthFields = array(0=>"5%",1=>"55%",2=>"40%"));
       $block1->openTablelist();

       for($i= 0;$i < $contHora;$i++){

           $block1->openRow("");
           $block1->cellRow("",'5%',"", '25');
           $block1->cellRow($blockPage->buildLink("horasmes.php?idreh=".$all_Hora->reh_id[$i],mostra_mes($all_Hora->reh_data[$i],$all_Hora->reh_data[$i]),in),'55%',"", '25');
           $block1->cellRow("<strong>&nbsp;".$all_Hora->reh_hora[$i]."h</strong>",'40%',"", '25');
           $block1->closeRow(); 
           //
