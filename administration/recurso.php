...
*/
 ...
 if (!$profilSession == 6) 
 {
  ...
  //FIM MOSTRA FUNCAO
      ...
      $blockPage->itemBreadcrumbs("Funções");
  ...
  echo "<a href='addrecursodesc.php?'><img  id=\"admm\" src=\"../themes/".THEME."/btn_add_norm.gif\" alt=\"Nova Função\" border=\"0\" onmouseover=\"trocaImg('btn_add_over.gif','admm');\" onmouseout=\"trocaImg('btn_add_norm.gif','admm');\" style=\"display:''\"></a>";

      $block1->openContent();
      $block1->arrayTitle($sortingFields = array(0=>"-",1=>"Função:",2=>"Profissional"),$widthFields = array(0=>"5%",1=>"55%",2=>"40%"));
      $block1->openTablelist();

      for($i= 0;$i < $contFunc;$i++){

         //INI MOSTRA SUBFUNCAO
         $tmp_Subfunc = " WHERE SUBF.nu_seq_funcao_sgd= '".$all_Funcao->func_id[$i]."' ";
         $Subfunc = new  request();
         $Subfunc->openSubfunc($tmp_Subfunc);
         $contSFunc = count($Subfunc->sfunc_id);

         //FIM MOSTRA SUBFUNCAO

         //INI MOSTRA USUARIO DA FUNÇÃO
         $tmp_Usu = " AND RU.nu_seq_sub_funcao_sgd='".$Subfunc->sfunc_id[0]."'";
         $sel_Usu = new  request();
         $sel_Usu->openRecurso_usuario($tmp_Usu);
         $contUsu = count($sel_Usu->rfunc_id);

         //FIM MOSTRA USUARIO DA FUNÇÃO



         $block1->openRow("");
         $block1->cellRow("",'5%',"", '25');//addsubrecursodesc.php
         $block1->cellRow($blockPage->buildLink("addsubrecursodesc.php?idrec=".$all_Funcao->func_id[$i],$all_Funcao->func_descricao[$i],in),'60%',"", '25');
         $block1->cellRow($blockPage->buildLink("recursousuario.php?fc=".$Subfunc->sfunc_id[0],"<strong>&nbsp;".$contUsu."</strong>",in),'35%',"", '25');
         $block1->closeRow(); 
         //
  ...
  echo "<span id=\"qtd\" style=\"height:20; border:0; position:absolute; right:140; border:0;\" >Total de Função: ".$somaUser."</span>";
