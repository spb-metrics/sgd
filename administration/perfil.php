...
//INI RETORNA A SIGLA DO PERFIL
function noPerfil($sessao) {
	switch ($sessao) {
		case 6 :
			$newperfil = 'sgd_coordenador_relacionamento';
			break;
		case 7 :
			$newperfil = 'sgd_analista_relat';
			break;
		case 1 :
			$newperfil = 'sgd_gerente_projeto';
			break;
		case 10 :
			$newperfil = 'sgd_coor_fabric';
			break;
		case 3 :
			$newperfil = 'sgd_gestor';
			break;
		case 11 :
			$newperfil = 'sgd_coordenador_relacionamento';
			break;
		case 8 :
			$newperfil = 'sgd_usuario';
			break;
...
if ($uperfil != "") {
	...
	$tmpcomplex = "UPDATE " . MYDATABASE . ".members SET profil='" . $newperfil . "' WHERE id = '$idSession' ";
	connectSql("$tmpcomplex");
	echo '<script>alert("ALTERAÇÃO REALIZADA \n \nNecessário Sair do Sistema!");</script>';
	//echo $newperfil;
	//headerFunction('../administration/perfil.php?'.session_name().'='.session_id());
...
$block1->contentTitle("Selecione o Perfil");
$block1->openTablelist();

for ($i = 0; $i < $contGuser; $i++) {
	$block1->openRow("");
	if (noPerfil($profilSession) == $all_Gusers->so_gnome[$i]) {
		$check = "checked";
		$aut = "<--- Seu Perfil Atual --->";
	} else {
		$check = " ";
		$aut = " ";
	}
	$block1->cellRow("<input name=\"radiobutton\" type=\"radio\" style=\"border:0 \" " . $check . " value=\"" . $all_Gusers->so_id[$i] . "\"  onClick=\"urlrede('uperfil=" . $all_Gusers->so_gnome[$i] . "','perfil.php?');\">", "10", "", "", "", "", "");
	$block1->cellRow("$aut", "60%", "", "", "", "");
	$block1->cellRow("<strong>" . $all_Gusers->so_gdesc[$i] . "</strong>", "35%", "", "", "", "");
	$block1->closeRow();
}
