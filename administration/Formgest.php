...
include('../includes/library.php');
 ...
 $id_usu = $_REQUEST['gsst'];

	function verificaIntegridade($id_sistema) {
	
		if ($id_sistema > 5000)
			$tag = '<a href="excluir.php?gsst='.$id_gestor.'&sis='.$co.'">Desvincular</a>';
		else 
			$tag = "";
	...
	include("../themes/".THEME."/header_window.php");	
  ...
  $block1->openRow("on");
  $block1->cellRow("","1%","","","0");
  $block1->cellRow("Nome:","10%","","","0","");
  $block1->cellRow($insNome,"89%","","","3","1");
  $block1->closeRow();	
  ...
  $block1->openRow("on");
  $block1->cellRow("","1%","","","0");
  $block1->cellRow("Email:","10%","","","0");
  $block1->cellRow($insEmail,"89%","","","3");
  $block1->closeRow();		
  
  
  $insLotacao = $block1->insInput("lotacaogs","lotacaogs","17","",$lotacao,"","17","disabled");
  $insLocalizacao = $block1->insInput("unigs","unigs","17","","","","17","disabled");
  
  $block1->openRow("on");
  $block1->cellRow("","1%","","","0");
  $block1->cellRow("Lotação:","10%","","","0");
  $block1->cellRow($insLotacao,"40%","","","0");
  $block1->cellRow("Localização:","10%","","","1");
  $block1->cellRow( $insLocalizacao,"40%","","","0");
  $block1->closeRow();	
  
  
  $insTelefone = $block1->insInput("tel","tel","17","",$tel,"","17","disabled");
  $insTelefoneatual = $block1->insInput("tel2","tel2","17","","","","17","disabled");
  
  $block1->openRow("on");
  $block1->cellRow("","1%","","","0");
  $block1->cellRow("Telefone:","10%","","","0");
  $block1->cellRow($insTelefone,"40%","","","0");
  $block1->cellRow("Telefone Atualizado:","10%","","","1");
  $block1->cellRow($insTelefoneatual,"40%","","","0");
  $block1->closeRow();
  ...
  $block1->openRow("on");
  $block1->cellRow("","10%","","","0");
  $block1->cellRow("<a href=\"FormAlt.php?id_usu=".$id_usu."\">|Alterar</a>|&nbsp;&nbsp; <a href=\"javascript:window.close();\">|X| Fechar</a>","90%","","","4","2");
  $block1->closeRow();
  ...
  $array_sistema = array(0=>"&nbsp;",1=>"Sigla",2=>"Nome do Sistema");
    
  $block1->openContent();
  $block1->arrayTitle($array_sistema,"100","left");
 
	   for($i=0; $i < $contSistema  ; $i++){
		 $link = verificaIntegridade($sel_Sistema->sis_id[$i]);	   	
	   
		 $block1->openRow("on");
		 $block1->cellRow("","2%","","2","0");
		 $block1->cellRow($sel_Sistema->sis_nome[$i],"15%","","2","0");
		 $block1->cellRow($sel_Sistema->sis_decricao[$i],"70%","","2","0");
	     $block1->closeRow();
	   } 	
