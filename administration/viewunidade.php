...
$block1->heading("Gerenciamento de Unidade");
$block1->form = "saP";
$block1->openForm("../administration/viewunidade.php?&" . session_name() . "=" . session_id() . "#" . $block1->form . "Anchor");
...
$block1->arrayTitle($sortingFields = array(0=>"-",1=>"Nome:",2=>"Quantidade de usuários"),$widthFields = array(0=>"5%",1=>"55%",2=>"40%"));
$block1->openTablelist();

for($i = 0 ; $i < $contUnidade;$i++ ){

	$sel_member = new  request();
	$tmquery_member = " WHERE mem.organization =".fixInt($sel_Unidade->org_id[$i]);
	$sel_member->openMembers($tmquery_member);
	$countMembers = count($sel_member->mem_id);
	
	$block1->openRow("");
	$block1->cellRow("",'5%',"", '25');
	$block1->cellRow($blockPage->buildLink("../administration/addunidade.php?change=edit&id=".$sel_Unidade->org_id[$i]."&",$sel_Unidade->org_name[$i],in),'60%',"", '25');
	$block1->cellRow("<strong>".$countMembers."</strong>",'35%',"", '25');
	$block1->closeRow();  
...
if ($profilSession == 6 || $profilSession == 0 || $profilSession == 9 || $profilSession == 7) {
	$block1->paletteScript(0, "add","../administration/addunidade.php?", "true,false,false",$strings["add"]);
	}
$block1->closePaletteScript($somaUser,"1");
//

echo "<span id=\"qtd\" style=\"height:20; border:0; position:absolute; right:154; border:0;\" >Total de Unidades: ".$contUnidade."</span>";
