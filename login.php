...
if ($auth == "on") 
{
	...
	{
		//inicio autentica oracle
		function autentica($USU,$PWSS,$APL,$GRU){
		$resultado ='';
		$res = '';
       // echo  $USU."--"; echo  $PWSS."--"; echo  $APL."--";  echo  $GRU."--";    

		//$db_conn = ocilogon("web_sgd", "sgd_web", "onix");MYSERVER
		$db_conn = ocilogon("web_sgd", "sgd_web",MYSERVER);
		
		$sq = 'BEGIN  segweb_fnde.autentica_usuario_pc(:bind1,:bind2,:bind3,:bind4,:bind5,:bind6); END;';
		
		//INTERPRETA 
		$sql = OCIParse($db_conn, $sq);
		
		//PASSA VARIÁVEIS PHP PARA O ORACLE 
		OCIBindByName($sql, ':bind1', $USU); 
		OCIBindByName($sql, ':bind2', $PWSS); 
		OCIBindByName($sql, ':bind3', $APL);
		OCIBindByName($sql, ':bind4', $GRU);
		OCIBindByName($sql, ':bind5', $resultado,132);
		OCIBindByName($sql, ':bind6', $res,132);
		...
		/*
		 $nrows = 	ocifetchstatement(	OCIExecute($sql, OCI_DEFAULT) , $results);
	   for ($i = 0; $i < $nrows; $i++ )
       {
        echo "<tr>\n";
        echo "<td>PASSOU ....".$results["$bind5"][$i]. "</td>";
        echo "</tr>\n";
    }   */

/*
		//echo "Resultado ........:$res --- $resultado";
		echo "usu: ".$USU."<br>";
		echo "pass: ".$PWSS."<br>";
		echo "apl: ".$APL."<br>";
		echo "g: ".$GRU."<br>";
		...
		echo "erro: ".$res."<br>";
		*/
		// echo "mostra ::".$resultado."<br>";
		
		return $resultado;
		
		} 

				function acesso($USU,$PWSS,$APL){
				$GRU = array(1=>'sgd_admin',2=>"sgd_analista_relat",3=>"sgd_coor_relat",4=>"sgd_coordenador_relacionamento",5=>"sgd_gerente_projeto",6=>"sgd_gestor",7=>"sgd_usuario",8=>"Sgd_coor_Fabric");
				$RES = array();
				$arg_true = 0; 
					//echo "passou antes loop---:".$val."<br>";
					for($i=1;$i <= count($GRU);$i++){
					//echo "passou depois loop---:".$val."<br>";
					
					$val = autentica(strtoupper($USU),$PWSS,$APL,$GRU[$i]);
					//echo "passou depois loop---:val:".$val;
						if($val=="TRUE"){
						//pega a posição referente ao grupo
						$arg_true = $i ; 
				//echo "tem true".$arg_true."<br>"; 
						//echo "passou ---:".$val."<br>";
						}
				...
				$au = "true";
				// autenticou usuario 
				//echo count($RES)."-- AUTENCICOU NO GRUPO : ".$GRU[$arg_true];
				//exit;
				//incio organização
				
				$tmplota = "  and u.ds_login ='".strtoupper($USU)."'";
				$testelota = new request();
				$testelota->openLotacao($tmplota);
				$contlota = count($testelota->lot_id);
				$lotacao = strtoupper($testelota->lot_lotado[0]);
					if($contlota > "0"){
					
					$tmporg = "  WHERE org.name LIKE '".$lotacao."'";
					$testeorg = new request();
					$testeorg->openOrganizations($tmporg);
					//echo "ORG: ".
					$contlota = count($testeorg->org_id);
						if($contlota <= "0"){
						//cadastra unidade 
						//echo "cadastra unidade ";
						$tmpcad = "INSERT INTO sgd_fnde.organizations ( name, address1, address2,created, extension_logo, owner, hourly_rate) VALUES ('".strtoupper($lotacao)."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','jpg','162','0')";
						connectSql($tmpcad);
						
						}else{
						
						$org = $testeorg->org_id[0];
						
						}
				}else{
				echo "nao esta lotado ";
				$au = "true";
				//exit;
				...
				$tmpquery = "WHERE mem.login = '".$USU."' AND mem.profil != '4'";
				$testeUser = new request();
				$testeUser->openMembers($tmpquery);
				//echo "<br>esta vazio ";
				 $countUser = count($testeUser->mem_id);
				
					if($countUser=="0"){
							$pass = get_password($PWSS);
							
							switch ($arg_true) {
							case 1:
							//admin
							//a.id, a.organization, a.login, a.password, a.name, a.title, a. a.email_home, a. a. a.mobile, a.fax, a.comments, a. a. a.logout_time,a.last_page, a.
							
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Administrador','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','0','0','0','projects/listprojects.php?')";
						    connectSql($tmpusua);
							//exit;
							//echo "i igual a 0";
							break;
							case 2:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Analista','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','7','0','0','projects_site/home.php?')";
						    connectSql($tmpusua);
							//echo "i igual a 1";
							break;
							case 3:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Relacionamento e Atendimento','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','9','0','0','projects/listprojects.php?')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							case 4:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Relacionamento','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','6','0','0','projects/listprojects.php?')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							case 5:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Gerente de projeto','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','1','0','0','projects_site/home.php')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							case 6:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Gestor','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','3','0','0','projects_site/home.php')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							case 7:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Usuário','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','8','0','0','projects_site/home.php')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							case 8:
							$tmpusua = "INSERT INTO members (name,title,password,organization,login,email_work,phone_work,phone_home,created,profil,timezone,logout_time,last_page) VALUES ('".$USU."','Usuário','$pass','".$org."','".$USU."','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".$USU."@fnde.gov.br','".date('Y-m-d')."','10','0','0','projects/listprojects.php?')";
						    connectSql($tmpusua);
							//echo "i igual a 2";
							break;
							default:
				...
				//$PWSS = $passwordForm;
				$APL = 'SGD';
				$ii = acesso(strtoupper($loginForm),$passwordForm,$APL);
				$match = $ii;
				//echo $ii."--mostra acesso"; 
		//fim autentica oracle
		
		
		 $tmpquery = "WHERE mem.login = '".strtoupper($loginForm)."' AND mem.profil != '4'";
	}
	...
	else 
	{
		...
		{
			//echo "<br>nao fez cookie.....";
			if (!$ssl && !is_password_match(strtoupper($loginForm), $passwordForm, $loginUser->mem_password[0])) 
			{
				$error = $strings["invalid_login"];
			//echo "<br>nao fez match.....".strtoupper($loginForm)."--".$loginUser->mem_password[0];
		...
		if ($match == true) 
		{
			...
			$loginSession = strtoupper($loginForm);
			$nameUser = $loginUser->mem_login[0];
			...
			$idOrganization = $loginUser->mem_organization[0];
			
			
			/*echo "id = ". $loginUser->mem_id[0]. "<br>";
			echo "login = ". $loginUser->mem_login[0]. "<br>";
			echo "senha = " .$loginUser->mem_password[0]. "<br>";
			echo " timezoneSession = ". $timezoneSession;*/

			
			//session patch by fullo 03/06/2003
			...
			$ip=$REMOTE_ADDR;
			$tmpquery = "WHERE log.login = '".strtoupper($loginForm)."'";
			...
			{
				//$tmpquery1 = "INSERT INTO ".$tableCollab["logs"]." VALUES('$loginForm','$passwordForm','$ip','$session','1','$dateheure','')";
				$tmpquery1 = "INSERT INTO ".$tableCollab["logs"]."(login,password,ip,s_session,compt,last_visite) VALUES('".strtoupper($loginForm)."','$passwordForm','$ip','$session','1','$dateheure')";
				connectSql("$tmpquery1");
			...
			else 
			{
				...
				//$tmpquery1 = "UPDATE ".$tableCollab["logs"]." SET ip='$ip',session='$session',compt='$increm',last_visite='$dateheure' WHERE login = '$loginForm'";
				 $tmpquery1 = "UPDATE ".$tableCollab["logs"]." SET ip='$ip', s_session='$session',compt='$increm',last_visite='$dateheure' WHERE login = '".strtoupper($loginForm)."'";
			...
			else if ($loginUser->mem_last_page[0] != "" && $loginUser->mem_profil[0] != "3" && $lastvisitedpage) 
			{
				...
				//$tmpquery = "UPDATE ".$tableCollab["members"]." SET last_page='' WHERE login = '".strtoupper($loginForm)."'";
				//connectSql("$tmpquery");
				
				
				//echo $loginSession ;
				//echo $loginUser->mem_last_page[0]." ".$lastvisitedpage;
				//exit();
				//echo "faz redirect";
				//exit;
			...
			//redirect to home or admin page (if user is administrator (Equipe de relacionamento))
			else 
			{
				if ($loginUser->mem_profil[0] == "3" || $loginUser->mem_profil[0] == "8" || $loginUser->mem_profil[0] == "1") 
				{
				...
				//modificado by Anderson® 05/05/2006
				else if (($loginUser->mem_profil[0] == "6") || ($loginUser->mem_profil[0] == "7") || ($loginUser->mem_profil[0] == "9") || ($loginUser->mem_profil[0] == "10"))
				{
					echo "faz redirect6";
				    exit;
...
//echo "<span style=\"color:#0000FF; font-weight: bold; font-size: 12px;\">".$strings["bemvindo"]."</span>";
if ($msg != "") 
{
...
//$block1->heading("PhpoiCollab : ".$strings["login"]);

//$block1->heading("SGD : ".$strings["login"]);

$block1->openContent();
$block1->contentTitle($strings["login"]);
...
// Substitui o select 
echo "<input value='pt-br' type='hidden' name='languageForm'>";
...
$block1->contentRow("","<input type=\"image\" name='save' value='".$strings["login"]."' src=\"../themes/default/images/botao_entrar.jpg\" style=\"width:75; height:22; border:0 \" ><br/><br/><br/>");
//.$blockPage->buildLink("../general/sendpassword.php?",$strings["forgot_pwd"],in)  dentro do submit                     
$block1->closeContent();
$block1->closeForm();
