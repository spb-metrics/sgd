		...
		if ($loginForm == "" && $passwordForm == "") {
			...
			if ($loginForm == "") {
				...
				if ($passwordForm == "") {
					...
					//$error = $strings["login_password2"];
					$error = "O nome de usuário e/ou a senha digita são inválidos. Por favor digite suas informações corretamente para entrar no sistema.";
					$auth = "on";
...
if ($auth == "on") {
	...
	if ($demoMode != true) {
		...
		function autentica($USU, $PWSS, $APL, $GRU) {
			...
			$tmp_aces = " AND SU.LOGIN='" . strtoupper($USU) . "' AND SU.PASSWORD='" . strtoupper($PWSS) . "' AND SG.NO_GRUPO='" . $GRU . "' ";
			$all_aces = new request();
			$all_aces->openSousuario($tmp_aces);
			$contaces = count($all_aces->so_id);
		...
		function acesso($USU, $PWSS, $APL) {
			$GRU = array (
				1 => 'sgd_admin',
				2 => "sgd_analista_relat",
				3 => "sgd_coor_relat",
				4 => "sgd_coordenador_relacionamento",
				5 => "sgd_gerente_projeto",
				6 => "sgd_gestor",
				7 => "sgd_usuario",
				8 => "sgd_coor_Fabric",
				9 => "sgd_user_serv"
			...
			*/
			...
			//	$gera->referencia($dateh,'2','100','Log de Autenticação',$oi);

			//INI LOTAÇÃO
			if ($arg_true > "0") {
				$au = "true";
						
				//ABAIXO INFORMA SE ESTA NO GRUPO DE SEGURANÇA E TEM PROFIL
				...
				if ($countUser == "0") {
					...
					$tmpusu = " AND a.ds_login = '" . $USU . "' ";
					$testeUsu = new request();
					$testeUsu->openUsuario($tmpusu);
					$nome_p = explode(" ", $testeUsu->usu_nome[0]);
					...
					if ($countUser == 1) {
						...
						//INI EXECUTAR CHECAGEM DE DOIS GRUPOS 
						if (count($aut_true) > 1) {

							switch ($testeUser->mem_profil[0]) {
								case 0 :
									$arg_true = 1;
									break;
								case 7 :
						...
						//FIM EXECUTAR CHECAGEM DE DOIS GRUPOS							
						...
						switch ($arg_true) {
							case 1 :
								//admin
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='projects/listprojects.php?', profil= '6' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);

								break;
							case 2 :
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='projects/listprojects.php?', profil= '7' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
								//echo "analista";
							
								break;
							case 3 :
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='reports/reportTemplate.php?', profil= '9' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
							...
							case 4 :
								...
								break;
							case 5 :
								$tmpusua = " UPDATE " . MYDATABASE . ".members SET last_page='projects_site/home.php?', profil= '1' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
								
								break;
							case 6 :
								$tmpusua = " UPDATE " . MYDATABASE . ".members SET last_page='projects_site/home.php?', profil= '3' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
							
								break;
							case 7 :
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='reports/reportTemplate.php?', profil= '8' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
							
								break;
							case 8 :
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='projects/listprojects.php?', profil= '10' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
								
								break;

							case 9 :
								$tmpusua = "UPDATE " . MYDATABASE . ".members SET last_page='projects_site/home.php?', profil= '11' WHERE ID = '" . $testeUser->mem_id[0] . "'";
								connectSql($tmpusua);
								break;

							default :
								echo "i não é igual a 0, 1 ou 2--" . $arg_true;
		...
		//$PWSS = $passwordForm;
		$APL = 'SGD';
		$Verifica_User = acesso(strtoupper($loginForm), $passwordForm, $APL);
		$match = $Verifica_User;
	...
	if ($comptLoginUser == "0") {
		...
		if ($match == true) {
			...
			$nome_usuario = explode(" ", $loginUser->mem_name[0]);
			$nameSession = $nome_usuario[0];
			...
			if ($comptRegisterLog == "0") {
				...
				$tmpquery1 = "INSERT INTO " . MYDATABASE . "." . $tableCollab["logs"] . "(login,ip,session,compt,last_visite) VALUES('" . strtoupper(trim($loginForm)) . "','$ip','$session','1','$dateheure')";
				connectSql("$tmpquery1");
				...
				$tmpquery1 = "UPDATE " . MYDATABASE . "." . $tableCollab["logs"] . " SET ip='$ip', session='$session',compt='$increm',last_visite='$dateheure' WHERE login = '" . strtoupper($loginForm) . "'";
				connectSql("$tmpquery1");
			...
			//redirect to last page required (with auto log out feature)
				...
				if ($loginUser->mem_last_page[0] != "" && $loginUser->mem_profil[0] != "3" && $lastvisitedpage) {
					...
					//principal 
					if ($loginUser->mem_profil[0] == 3 || $loginUser->mem_profil[0] == 1) {
						$tmpquery = "UPDATE " . MYDATABASE . "." . $tableCollab["members"] . " SET last_page='projects_site/home.php?' WHERE login = '" . strtoupper($loginForm) . "'";
						connectSql("$tmpquery");
					}
					//principal 
					if ($loginUser->mem_profil[0] == 8) {
						$tmpquery = "UPDATE " . MYDATABASE . "." . $tableCollab["members"] . " SET last_page='reports/reportTemplate.php?' WHERE login = '" . strtoupper($loginForm) . "'";
						connectSql("$tmpquery");
					}

					//
					if ($loginUser->mem_profil[0] == 6 || $loginUser->mem_profil[0] == 7 || $loginUser->mem_profil[0] == 10 || $loginUser->mem_profil[0] == 9) {
						$tmpquery = "UPDATE " . MYDATABASE . "." . $tableCollab["members"] . " SET last_page='projects/listprojects.php?' WHERE login = '" . strtoupper($loginForm) . "'";
						connectSql("$tmpquery");
					}
			...
			//redirect to home or admin page (if user is administrator (Equipe de relacionamento))
				...
				if ($loginUser->mem_profil[0] == "3" || $loginUser->mem_profil[0] == "8" || $loginUser->mem_profil[0] == "1") {
					...
					if ($loginUser->mem_profil[0] == "0") {
						...
						if (($loginUser->mem_profil[0] == 6) || ($loginUser->mem_profil[0] == 7) || ($loginUser->mem_profil[0] == 9) || ($loginUser->mem_profil[0] == 10)) {
...
$block1->contentRow(" " . $strings["password"], "<input value='' type='password' name='passwordForm'>");
