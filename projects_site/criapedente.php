...
//echo "ok".$tipo."--".$idfase."--".$projectSession;

if(!$textarea==""){
$tmpmail2 = " WHERE pro.id = '".$projectSession."' ";
$userpro = new request();
$userpro->openProjects("$tmpmail2");

$tmpanexo = "INSERT INTO ".$tableCollab["fase_pedente"]." (id_fase,id_projeto,descricao) VALUES('$idfase','$projectSession','$textarea')";
connectSql("$tmpanexo");
//echo "ok<br>".$tipo."--".$idfase."--".$projectSession;
//exit;
switch($tipo){
	case 1:
	$nome = "Planejamento";
...
//envia email 
	...
	//if($m_impla==2){
		...
		$tmpmail = " AND (mem.profil= '10' or mem.id= '".$userpro->pro_men_analista[0]."' or mem.profil= '6') ";
		//exit;
		...
		$contmail = count($usermail->mem_id);
			...
			$envimail = new email();
			$envimail->enviar('Pendente','Demanda '.$projectSession.'-'.$userpro->pro_name[0].' <br>Fase '.$nome.' Está paralisada devido a uma situação de contingência.', $usermail->mem_email_work[$um]);
			...
			//GERA REFERENCIA
             		$gera->referencia($dateh,'1',$projectSession,'Gerado Pendência de Fase',$oi);
			
			//INI AÇÃO 
			$dateheure2 = invert_data($dateh)." ".$hora.$minu;
			$acao = new arquivo();
			$faz = $acao->gera($dat,$hor.date('B'));
			$fazz = $acao->insert($dateheure2." EXECUTADO POR GERENTE DE PROJETO: ".$nameSession." <br>AÇÃO : FASE ".$nome." PENDENTE - .<br><br>",$faz);
			$acao->referencia($dateh,'3',$projectSession,'Fase '.$nome.' Pendente',$faz);
			unset($fazz);
			unset($faz);
			unset($acao);
			//FIM AÇÃO
...
//exit;
//echo "fez";
/*echo "<script>window.opener.document.location.href='home.php?id=".$id_pro."';</script><br>";*/
echo '<script>alert("Registrado com Sucesso.\n Agora Click em Confirmar!");</script>';
echo '<script>window.close();</script>';


}else{

//echo "Informe os dados nescessários !";
...
<script language='JavaScript' src='../themes/default/scripts.js'></script>
...
<form name="form1" method="post" action="">
  <table width="200" height="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr class="old">
      ...
      <td width="174" height="20"><strong>Justificativa da Pend&ecirc;ncia </strong></td>
    ...
    <tr>
      <td>&nbsp;</td>
      <td><textarea name="textarea" cols="30" rows="3" wrap="VIRTUAL" class="text_10" onKeyUp="ContaCaracteres('textarea','textconta','sBann',2000);"></textarea></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td height="10"></td>
      <td height="10"><span id="sBann" class="minitext">2000 caracteres.</span>
       <input type="hidden" name="textconta"><input type="hidden" name="tipo" value="<?php echo $tipo; ?>"><input type="hidden" name="idfase" value="<?php echo $idfase; ?>"></td>
      <td height="10"></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td align="center"><input name="Submit" type="submit" class="text_10" value="Enviar"></td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td height="10" colspan="3"></td>
