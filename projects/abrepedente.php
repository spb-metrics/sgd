...
if(!$idpend==""){
	$tmpquery = "WHERE fasp.id_fase = '".$idpend."'";
	$faseJustificativa = new request();
	$faseJustificativa->openFasep($tmpquery);
	$Justificativa = $faseJustificativa->fasp_descricao[0];
}

$block1->openContent();

$block1->contentTitle("Justificativa da Pend&ecirc;ncia");
...
$block1->openRow("on");
$block1->cellRow("$Justificativa","","","82","2","2");
$block1->closeRow();

$block1->openRow("on");
$block1->cellRow($block1->buildLink("Javascript:window.close();","<strong>Fechar</strong>","out"),"","","10","2","2");
$block1->closeRow();
