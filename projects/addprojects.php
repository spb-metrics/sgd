...
if ($profilSession == 4 || $profilSession == 3 || $profilSession == 1 || $profilSession == 8 || $profilSession == 10) {
	...
	headerFunction("listprojects.php?&changeProject=true&" . session_name() . "=" . session_id());
...
$tmp2query = "AND mem.profil = '0'";
$usersel = new request();
$usersel->openMembers($tmp2query);
$usermo = count($usersel->mem_id);
...
if ($action == "add") {
	...
	if ($select == "0") {
		echo '<script>alert("Selecione o Sistema!!");</script>';
	...
	$tmpquery1 = "INSERT INTO " . $tableCollab["projects"] . "(name,priority,description,owner,organization,status,created,published,upload_max,tipo_demanda,prazo_status,id_sistema,hourly_rate,phase_set,invoicing " . $tmpan . ") VALUES('$pn','$pr','$d','$user','$morg','" . $tmpst . "','$dateheure1','0','51200','$tdem',1,'$select','1','1','0' " . $tmpan1 . ")";
	...
	$tmpquery2 = "INSERT INTO " . $tableCollab["teams"] . "(project,member,published,authorized) VALUES('$projectNew','$idSession','1','0')";

	for ($z = 0; $z <= count($usermo); $z++) {
		$userint = $usermo[$z];

		$tmp3query1 = "WHERE tea.project = '$projectNew' and member = '$userint' ";
		$teansel = new request();
		$teansel->openTeams($tmp3query1);
		$teanuser = count($teansel->tea_member);

		if ($teanuser < 1) {
			$tmp2query5 = "INSERT INTO " . $tableCollab["teams"] . "(project,member,published,authorized) VALUES('$projectNew','$userint','1','0')";
		}
	...
	if ($tdem == 1) {
		...
		headerFunction("adderro.php?project=$projectNew&updateProject=true");
		exit;
	} else {

		headerFunction("viewproject.php?id=$projectNew&updateProject=true");
		exit;
	}
}
...
$block1 = new block();
$block1->heading($strings["n_demandas"]);
...
$selectSistemas = $block1->insSelect($Sistemas->sis_id, $Sistemas->sis_unidade, "", "select", "select", "", $OptSistemas, "", "17", "onChange=\"foco_campo('pn')\";");
...
$rowPrioridades = count($Prioridades->so_id);
$OptPrioridades = array (
	0 => "0",
	1 => "<strong>Selecione Prioridade</strong>"
);
$selectPrioridades = $block1->insSelect($Prioridades["value"], $Prioridades["label"], "", "pr", "pr", "", $OptPrioridades, "", "17", "");
//FIM SELECT DE PRIORIDADE

$block1->openForm("../projects/addprojects.php?" . session_name() . "=" . session_id() . "&action=add&project=$projectSession#filedetailsAnchor", "addsupport", "onSubmit='return Valida(this);'");
...
$block1->openRow("on");
$block1->cellRow("" . $strings["sistema"] . " :", "70", "", "", "", "1");
$block1->cellRow("$selectSistemas", "135", "", "", "", "");
$block1->cellRow("<input type=\"checkbox\" id=\"check\" name=\"check\" value=\"1\" border=\"0\" style=\"border:0 ; border-color:blank;\" onClick=\"mostra_campo('campofile','cerro','check')\";>", "20", "", "", "", "");
$block1->cellRow("Erro", "40", "", "", "", "");
$block1->cellRow("<input type=\"checkbox\" id=\"checkbox\" name=\"checkbox\" value=\"0\" border=\"0\" style=\"border:0 ; border-color:blank;\" onClick=\"desabilita('select','novs')\";>", "20", "", "", "", "");
$block1->cellRow("Novo Sistema", "90", "", "", "", "");
$block1->cellRow("<input type=\"checkbox\" id=\"chec\" name=\"chec\" value=\"3\" border=\"0\" style=\"border:0 ; border-color:blank;\" onClick=\"desabilita2('select','serv')\";>", "20", "", "", "", "");
$block1->cellRow("Serviços", "270", "", "", "", "");
$block1->closeRow();

$block1->openRow("on");
$block1->cellRow("" . $strings["titulo"] . " :", "", "", "", "", "1");
$block1->cellRow("" .
"<input size=\"32\" value=\"\" style=\"width: 250px\" name=\"pn\" maxlength=\"32\" type=\"TEXT\">" .
"<input type=\"hidden\" id='campofile' name=\"campofile\" value=\"0\">", "", "", "", "7", "");
$block1->closeRow();

$block1->openRow("on");
$block1->cellRow("" . $strings["priority"] . " :", "", "", "", "", "1");
$block1->cellRow("$selectPrioridades", "", "", "", "7", "");
$block1->closeRow();
...
$block1->openRow("on");
$block1->cellRow("&nbsp;", "", "", "", "", "");
$block1->cellRow("<input type=\"SUBMIT\" value='" . $strings["save"] . "'>", "", "", "", "7", "");
$block1->closeRow();

echo "<input type=\"hidden\" id=\"serv\" name=\"serv\" value=\"0\">" .
"<input type=\"hidden\" id=\"novs\" name=\"novs\" value=\"0\">" .
"<input type=\"hidden\" id=\"select_analista\" name=\"select_analista\" value=\"$idSession\">" .
"<input type=\"hidden\" id=\"user\" name=\"user\" value=\"$idSession\">" .
"<input type=\"hidden\" id=\"morg\" name=\"morg\" value=\"$menber_org\">" .
"<input type=\"hidden\" id=\"mogo\" name=\"mogo\" value=\"$teanuser\">";
