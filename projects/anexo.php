...
if (!$text_titulo == "") {
	...
	if (!$_FILES['text_doc']['name'] == "") {
		if ($_FILES['text_doc']['size'] > 1000000) {
			echo '<script>alert("arquivo fora do padrão (maior que 10000 Kbytes)!");history.back();</script>';
			exit ();
		}
		$dat = date('Y-m-d');
		$arquivo = $_FILES['text_doc']['name'];
		$fo = $_FILES['text_doc']['tmp_name'];
		$fp = fopen($_FILES['text_doc']['tmp_name'], "rb");
		$img = fread($fp, filesize($_FILES['text_doc']['tmp_name']));
		fclose($fp);
		$img = addslashes($img);

		$uploaddir = "../anexo/" . trim($id_pro) . "/";
		$diretorio = "../anexo/" . trim($id_pro) . "/";

		$up = $diretorio . basename($_FILES["text_doc"]['name']);

		if (move_uploaded_file($_FILES['text_doc']['tmp_name'], $up)) {
			move_uploaded_file($fot, $up);
		} else {
			echo "Erro ao subir o ficheiro. Não se pode guardar o ficheiro.";
			echo '<script>alert("Erro ao subir o ficheiro. Não se pode guardar o ficheiro");history.back();</script>';
			exit;
		}
	}

	$tmpanexo = "INSERT INTO " . $tableCollab["ata_anexo"] . " (titulo,documento,id_ata,data) VALUES ('$text_titulo','" . $up . "','$id_ataa','" . date('Y-m-d') . "')";

	connectSql($tmpanexo);

	echo '<script>alert("Registrado com Sucesso!");</script>';
	echo "<script>window.opener.document.location.href='editproject.php?id=" . $id_pro . "';</script><br>";
...
$block1->openForm("anexo.php");
$block1->openContent();

$block1->contentTitle("Anexo de Escopo","4");
...
$block1->openRow("on");
$block1->cellRow("&nbsp;","","","","","");
$block1->cellRow("Titulo","","","","","");
$block1->cellRow("<input name=\"text_titulo\" type=\"text\" class=\"\" size=\"45\" maxlength=\"40\"/>","","","","2","");
$block1->closeRow();

$block1->openRow("on");
$block1->cellRow("&nbsp;","","","","","");
$block1->cellRow("Anexo","","","","","");
$block1->cellRow("<input name=\"text_doc\" type=\"file\" class=\"\" size=\"40\">","","","","2","");
$block1->closeRow();
...
$block1->openRow("on");
$block1->cellRow("<input name=\"Submit\" type=\"submit\" class=\"\" value=\"Salvar\">","","","","4","2");
$block1->closeRow();

$block1->closeContent();
$block1->closeForm();

include('../themes/'.THEME.'/footer_window.php');
?>