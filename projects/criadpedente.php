...
if(!$textarea == "") {
	...
	$tmpanexo = " UPDATE ".MYDATABASE.".projects SET  description='$textarea' where id='$projec' ";
	connectSql("$tmpanexo");
	...
	//envia email 
	if($tipos!=12) {
		$tmp =  " or mem.id= '".$userpro->pro_owner[0]."' ";
	} else {
		$tmp = "";
	}
	 
	    $tmpmail = " AND (mem.profil= '10' or mem.id= '".$userpro->pro_men_analista[0]."' or mem.profil= '6' ".$tmp." ) ";
		...
		$contmail = count($usermail->mem_id);
			...
			for($um=0; $um < $contmail;$um++){
				$envimail = new email();
				$envimail->enviar('Demanda '.$nome.'','Demanda '.$projec.'- '.$userpro->pro_name[0].' <br>Está '.$nome.' devido a uma situação de contingência.', $usermail->mem_email_work[$um]);
				unset($envimail);		
			}
	...
	$block1->contentTitle("Justificativa","3");
	
	$block1->openRow("on");
	$block1->cellRow("<textarea id=\"textarea\" name=\"textarea\" style=\"height:52px;\" cols=\"36\" rows=\"8\" wrap=\"VIRTUAL\" onKeyUp=\"ContaCaracteres('textarea','textconta','sBann',255);\"></textarea>" .
			"<br/><span id=\"sBann\">255</span> caracter(es)." .
