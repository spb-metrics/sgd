...
if(!$textarea==""){
	$tmpmail2 = " WHERE pro.id = '".$projec."' ";
	$userpro = new request();
	$userpro->openProjects("$tmpmail2");

	$tmpanexo = "INSERT INTO ".$tableCollab["fase_pedente"]." (id_fase,id_projeto,descricao) VALUES('$idfase','$projec','$textarea')";
	connectSql("$tmpanexo");
	...
	//envia email 
	    ...
	    echo $tmpmail = " AND (mem.profil= '10' or mem.id= '".$userpro->pro_men_analista[0]."' or mem.profil= '6') ";
		...
		for($um=0; $um < $contmail;$um++){
			$envimail = new email();
			$envimail->enviar('Pendente','Demanda '.$projec.'- '.$userpro->pro_name[0].' <br>Fase '.$nome.' Está paralisada devido a uma situação de contingência.', $usermail->mem_email_work[$um]);
			unset($envimail);
		}
		
		unset($usermail);
	
	echo '<script>alert("Registrado com Sucesso\nClick em Salvar!");</script>';
	echo '<script>window.close();</script>';
}
	$block1->openForm();
	$block1->openContent("200");

	$block1->contentTitle("Justificativa da Pend&ecirc;ncia","3");
	
	$block1->openRow("on");
	$block1->cellRow("<textarea id=\"textarea\" name=\"textarea\" cols=\"36\" rows=\"9\" style=\"height:52px;\" wrap=\"VIRTUAL\" onKeyUp=\"ContaCaracteres('textarea','textconta','sBann',2000);\"></textarea>" .
			"<br/><span id=\"sBann\">2000</span> caracter(es)." .
			"<input type=\"hidden\" name=\"textconta\" id=\"textconta\">" .
			"<input type=\"hidden\" name=\"tipo\" id=\"tipo\" value=\"$tipo\">" .
			"<input type=\"hidden\" name=\"idfase\" id=\"idfase\" value=\"$idfase\">" .
			"<input type=\"hidden\" name=\"projec\" id=\"projec\" value=\"$proj\">","","","","3","");
	$block1->closeRow();	

	$block1->openRow("on");
	$block1->cellRow("<input name=\"Submit\" type=\"submit\" value=\"Enviar\">","","","10","3","2");
	$block1->closeRow();
