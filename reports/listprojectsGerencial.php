...
if (($fillter == "true") && ($fill != "")) {

	for ($p = 1; $p < count($status_s); $p++) {
		$informa = $status_s[$p];
		if (strtoupper($informa) == strtoupper($fill)) {
			$pe = $p;
		}
	}
	$pes = $pe;
	$_SESSION['filter'] = trim($fill);
	headerFunction("../reports/listprojects.php?" . session_name() . "=" . session_id());
}

$opcao = $_REQUEST['opcao'];

if ($_REQUEST['un'] != "") {
	$_SESSION['id_unidade'] = $_REQUEST['un'];
} else {
	$_SESSION['id_unidade'] = $_SESSION['id_unidade'];
}

if ($_REQUEST['si'] != "") {
	$_SESSION['id_sistema'] = $_REQUEST['si'];
} else {
	$_SESSION['id_sistema'] = $_SESSION['id_sistema'];
}

if ($_REQUEST['gs'] != "") {
	$_SESSION['id_gestor'] = $_REQUEST['gs'];
} else {
	$_SESSION['id_gestor'] = $_SESSION['id_gestor'];
}

if ($_REQUEST['id_prioridade'] != "") {
	$_SESSION['id_prioridade'] = $_REQUEST['id_prioridade'];
} else {
	$_SESSION['id_prioridade'] = $_SESSION['id_prioridade'];
}

if ($_REQUEST['id_status'] != "") {
	$_SESSION['id_status'] = $_REQUEST['id_status'];
} else {
	$_SESSION['id_status'] = $_SESSION['id_status'];
}

if ($_REQUEST['grprojeto'] != "") {
	$_SESSION['id_ger_pro'] = $_REQUEST['grprojeto'];
} else {
	$_SESSION['id_ger_pro'] = $_SESSION['id_ger_pro'];
}

if ($_REQUEST['arel'] != "") {
	$_SESSION['id_relacionamento'] = $_REQUEST['arel'];
} else {
	$_SESSION['id_relacionamento'] = $_SESSION['id_relacionamento'];
}

if ($_REQUEST['gerprona'] != "") {
	$_SESSION['gerprona'] = $_REQUEST['gerprona'];
} else {
	$_SESSION['gerprona'] = $_SESSION['gerprona'];
}

if ($_REQUEST['id_situacao'] != "") {
	$_SESSION['id_situacao'] = $_REQUEST['situacao'];
} else {
	$_SESSION['id_situacao'] = $_SESSION['id_situacao'];
}

if ($_REQUEST['tipo_demanda'] != "3") {
	$_SESSION['id_tipo_demanda'] = $_REQUEST['tipo_demanda'];
} else {
	$_SESSION['id_tipo_demanda'] = $_SESSION['tipo_demanda'];
}

if ($_REQUEST['ms'] != "") {
	$_SESSION['id_ms'] = $_REQUEST['ms'];
} else {
	$_SESSION['id_ms'] = $_SESSION['id_ms'];
}

if ($_REQUEST['an'] != "") {
	$_SESSION['id_ano'] = $_REQUEST['an'];
} else {
	$_SESSION['id_ano'] = $_SESSION['id_ano'];
}
...
function sistema($id) {
	...
	for ($i = 0; $i < $comp; $i++) {
		...
		if ($listSistem->sis_id[$i] == $id) {
			$nome[0] = $listSistem->sis_unidade[$i];
			$nome[1] = $listSistem->sis_decricao[$i];
		}
	}
...
$block1->form = "saP";
$block1->openForm("../reports/listprojects.php?typeProjects=$typeProjects&" . session_name() . "=" . session_id() . "#" . $block1->form . "Anchor");

$block1->heading($strings["demandas"]);

$block1->openPaletteIcon();
if ($profilSession == 0 || $profilSession == 9 || $profilSession == 6 || $profilSession == 7) {
}
...
$block1->rowsLimit = "10";
$block1->sorting("projects", $sortingUser->sor_projects[0], "pro.id DESC", $sortingFields = array (
	0 => "pro.id",
	1 => "pro.name",
	2 => "pro.priority",
	3 => "org.name",
...
$where = "";
$possui_condicao = false;

if ($_SESSION['id_ano'] != '') {
	$possui_condicao = true;
	$where = "WHERE $years = " . $_SESSION['id_ano'] . " ";
}

if ($_SESSION['id_ms'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND $month = " . $_SESSION['id_ms'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE $month = " . $_SESSION['id_ms'] . " ";
	}
}

if ($_SESSION['id_prioridade'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.priority = " . $_SESSION['id_prioridade'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.priority = " . $_SESSION['id_prioridade'] . " ";
	}
}

if ($_SESSION['id_unidade'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.organization = " . $_SESSION['id_unidade'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.organization = " . $_SESSION['id_unidade'] . " ";
	}
}
if ($_SESSION['id_gestor'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.owner = " . $_SESSION['id_gestor'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.owner = " . $_SESSION['id_gestor'] . " ";
	}
}

if ($_SESSION['id_status'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.prazo_status = " . $_SESSION['id_status'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.prazo_status = " . $_SESSION['id_status'] . " ";
	}
}

if ($_SESSION['id_sistema'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.id_sistema = " . $_SESSION['id_sistema'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.id_sistema = " . $_SESSION['id_sistema'] . " ";
	}
}

if ($_SESSION['id_tipo_demanda'] != '0') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.tipo_demanda = " . $_SESSION['id_tipo_demanda'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.tipo_demanda = " . $_SESSION['id_tipo_demanda'] . " ";
	}
} else {
	$tipo = "AND pro.tipo_demanda IN ('1','2','3') ORDER BY pro.id DESC";
}

if ($_SESSION['id_situacao'] != '') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.status = " . $_SESSION['id_situacao'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.status = " . $_SESSION['id_situacao'] . " ";
	}
}

if ($_SESSION['gerprona'] == '41') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.id_ger_pro IS NULL ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.id_ger_pro IS NULL ";
	}
}
if ($_SESSION['gerprona'] == '42') {
	if ($possui_condicao == true) {
		$where = " $where AND pro.id_ger_pro IS NOT NULL ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.id_ger_pro IS NOT NULL ";
	}
}

if ($_SESSION['id_ger_pro'] >= 1) {
	if ($possui_condicao == true) {
		$where = " $where AND pro.id_ger_pro = " . $_SESSION['id_ger_pro'] . " ";
	} else {
		$possui_condicao = true;
		$where = " WHERE pro.id_ger_pro = " . $_SESSION['id_ger_pro'] . " ";
	}
}

$tmpqueryy = "";
$tmpqueryy .= "$where $tipo";

$block1->recordsTotal = compt($initrequest["projects"] . " " . $tmpqueryy);

$listProjects = new request();
$listProjects->openProjects($tmpqueryy, $block1->borne, $block1->rowsLimit);
$comptListProjects = count($listProjects->pro_id);
$paginador = intval($comptListProjects / 10);
$resto = $comptListProjects % 10;
...
if ($comptListProjects != 0) {
	...
	$block1->labels($labels = array (
		...
		1 => $strings["sistema"],
		2 => $strings["demanda"],
		3 => $strings["tipo_demanda"],
		4 => $strings["priority"],
		5 => $strings["status"],
		6 => $strings["statuss"]
	), "true");
	$total2 = intval($borne1 +10) / 10;
	...
	for ($i = $borne1; $i < $borne1 + $tot; $i++) {
		if ($listProjects->pro_org_id[$i] == "1") {
			$listProjects->pro_org_name[$i] = $strings["none"];
		}
		$tipoDemanda = $listProjects->pro_t_demada[$i];
		$ll = $listProjects->pro_sistema[$i];
		if (!$ll == "") {
		...
		if ($tipoDemanda == 0) {
			$demandaSistema = "Novo Sistema";
		} else {
			$demandaSistema = "Correção";
		}
		$idStatus = $listProjects->pro_status[$i];
		$t_demanda = $listProjects->pro_t_demada[$i];
		$idPriority = $listProjects->pro_priority[$i];
		$statuss = pega_status($listProjects->pro_id[$i]);
		$block1->openRow();

		if (($idStatus == 11 || $idStatus == 2 || $idStatus == 12) && ($listProjects->pro_description[$i] != "")) {
			$imgs = "<img src=\"../themes/default/exclamacao_" . $idStatus . ".gif\" width=\"12\" height=\"12\" title=\"Click para Visualizar a Justificativa \" border=\"0\" align=\"bottom\" cursor:hand style=\"display:''; cursor:hand\" onClick=\"janelada('../projects/abredpedente.php?proj=" . $listProjects->pro_id[$i] . "','pedente','250','160');\">";
		} else {
			$imgs = "";
		}
		$block1->cellRow($imgs, "", "", 18);

		$block1->cellRow($listProjects->pro_id[$i]);
		$block1->cellRow("<span title=\"" . $system[1] . "\" style=\"cursor:hand \">" . $system[0] . "</span>");
		//$block1->cellRow($blockPage->buildLink("../projects/viewproject.php?id=".$listProjects->pro_id[$i],$listProjects->pro_id[$i],in));
		if ($profilSession == 1) {
			if ($opcao == 1) {
				$block1->cellRow($blockPage->buildLink("../projects_site/home.php?id=" . $listProjects->pro_id[$i], $listProjects->pro_name[$i], in), '32%');
			} else {
				$block1->cellRow($blockPage->buildLink("../reports/relProjectsPDF.php?id=" . $listProjects->pro_id[$i], $listProjects->pro_name[$i], inblank), '32%');
			}
		} else {
			if ($opcao == 1) {
				$block1->cellRow($blockPage->buildLink("../projects/viewproject.php?id=" . $listProjects->pro_id[$i], $listProjects->pro_name[$i], in), '32%');
			} else {
				$block1->cellRow($blockPage->buildLink("../reports/relProjectsPDF.php?id=" . $listProjects->pro_id[$i], $listProjects->pro_name[$i], inblank), '32%');
			}
		}

		$block1->cellRow($tipo_demanda[$t_demanda]);
		$block1->cellRow("<img src=\"../themes/" . THEME . "/gfx_priority/" . $idPriority . ".gif\" alt=\"\"> " . $priority[$idPriority], '10%');
		$block1->cellRow($status[$idStatus], '15%');

		if ($sitePublish == "true") {
			if ($listProjects->pro_published[$i] == 1) {
				$block1->cellRow("<img src=\"../themes/" . THEME . "/gfx_priority/" . $statuss . ".gif\" alt=\"\">&nbsp;" . $status_s[$statuss]);
			} else {
				$block1->cellRow("<img src=\"../themes/" . THEME . "/gfx_priority/" . $statuss . ".gif\" alt=\"\">&nbsp;" . $status_s[$statuss]);
			}
		}
	...
	if (!$situa == "") {
		$typeProjects .= "&situa=" . $situa;
	}
	$block1->bornesPages("1", $blockPage->bornesNumber, "", "$typeProjects", "$ind", "10");
} else {
	$block1->noresults();
}
print "Total de Demandas: " . $comptListProjects;
if ($situa == "") {
	echo "<span id=\"qtd\" style=\"height:20; border:0; position:absolute; right:170; border:0;\" >Justificativa: <img src=\"../themes/default/exclamacao_12.gif\" width=\"12\" height=\"12\" border=\"0\" cursor:hand style=\"display:''; cursor:hand\" > - Pendente <img src=\"../themes/default/exclamacao_2.gif\" width=\"12\" height=\"12\" border=\"0\" cursor:hand style=\"display:''; cursor:hand\" > - Suspensa </span>";
} else {
	echo "<span id=\"qtd\" style=\"height:20; border:0; position:absolute; right:170; border:0;\" >Justificativa: <img src=\"../themes/default/exclamacao_11.gif\" width=\"12\" height=\"12\" border=\"0\" cursor:hand style=\"display:''; cursor:hand\" > - Cancelada</span>";
}
