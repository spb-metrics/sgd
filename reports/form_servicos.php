...
$blockServicos1 = new block();
$blockServicos2 = new block(); 
$blockPageServicos = new block();

//SELECT DE UNIDADES
	$tmpquery = "ORDER BY org.id";
	$Unidades = new request();
	$Unidades->openOrganizations($tmpquery);
	$rowUnidades = count($Unidades->org_id); 
	$OptUnidades = array(0=>"0",1=>"<strong>Selecione Unidade</strong>");
	$selectUnidades = $blockPageServicos->insSelect($Unidades->org_id,$Unidades->org_name,"","uns","uns","",$OptUnidades,"","17","");
...
//SELECT DE GESTORES
	...
	$OptGestores = array(0=>"0",1=>"<strong>Selecione Gestor</strong>");
	$selectGestores = $blockPageServicos->insSelect($Gestores->so_id,$Gestores->so_nome,"","gs","gs","",$OptGestores,"","17","");
//FIM SELECT DE GESTORES

//SELECT DE RESPONSAVEL
	$tmpquery = "AND SG.NO_GRUPO IN('sgd_user_serv')";
	$Responsaveis = new request();
	$Responsaveis->openSousuario($tmpquery);
	$rowResponsaveis = count($Responsaveis->so_id);
	$OptResponsaveis = array(0=>"0",1=>"<strong>Selecione Resposável da Área</strong>");
	$selectResponsaveis = $blockPageServicos->insSelect($Responsaveis->so_id,$Responsaveis->so_nome,"","resparea","resparea","",$OptResponsaveis,"","17","");
//FIM SELECT DE RESPONSAVEL

//SELECT DE SITUAÇOES
	$Situacoes = array("value" => array(3,8,7,11,10,1,6,9,12,2,0,4,5),
	"label" => array("Aberto","Aceito pelo Cliente","Aguardando Homologação","Cancelada","Concluído","Em Analise","Em Andamento","Em Homologação","Pendente","Suspensa","Validado pelo Cliente","Validado pelo relacionamento","Validado TI"));
	$OptSituacoes = array(0=>"20",1=>"<strong>Selecione Situação</strong>");
	$selectSituacoes = $blockPageServicos->insSelect($Situacoes["value"],$Situacoes["label"],"20","situacao","situacao","",$OptSituacoes,"","17","");
//FIM SELECT DE SITUAÇOES

//SELECT DE STATUS
	$Status = array("value" => array(1,2,3,5),"label" => array("A definir","Em dia","Em atraso","Replanejar"));
	$OptStatus = array(0=>"0",1=>"<strong>Selecione Status</strong>");
	$selectPrioridades = $blockPageServicos->insSelect($Status["value"],$Status["label"],"","id_status","id_status","",$OptStatus,"","17","");
//FIM SELECT DE STATUS

//SELECT DE PRIORIDADE
	$Prioridades = array("value" => array(2,3,5),"label" => array("Baixa","Media","Alta"));
	$OptPrioridades = array(0=>"0",1=>"<strong>Selecione Prioridade</strong>");
	$selectStatus = $blockPageServicos->insSelect($Prioridades["value"],$Prioridades["label"],"","id_prioridade","id_prioridade","",$OptPrioridades,"","17","");
//FIM SELECT DE PRIORIDADE

//SELECT DE AREA RESPONSAVEL
	$AreaResponsavel = array("value" => array(7,8,5,6),"label" => array("Gestão de Conhecimento","Infraestrutura","Produção","Web"));
	$OptAreaResponsavel = array(0=>"0",1=>"<strong>Selecione Área Responsável</strong>");
	$selectAreaResponsavel = $blockPageServicos->insSelect($AreaResponsavel["value"],$AreaResponsavel["label"],"","id_resp_area","id_resp_area","",$OptAreaResponsavel,"","17","");
//FIM SELECT DE AREA RESPONSAVEL

//SELECT DE ANO DEMANDA
	$tmpquery = "";
	$Anos = yearProjects();
	$Anos = array_clear($Anos);
	$OptAnos = array(0=>"0",1=>"<strong>Selecione Ano da Demanda</strong>");
	$selectAnos = $blockPageServicos->insSelect($Anos,$Anos,"","an","an","",$OptAnos,"","17","");
//FIM SELECT DE ANO DEMANDA


//INICIO PRIMEIRO FORMULARIO
$blockServicos1->openForm("listprojectsServicos.php");
$blockServicos1->openContent();

$blockServicos1->contentTitle("Selecione os critérios do relatório aqui.","4");	
	
$blockServicos1->openRow("on");
$blockServicos1->cellRow("","","","","4");
$blockServicos1->closeRow();

$blockServicos1->openRow("on");
$blockServicos1->cellRow("Status :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectSituacoes","","","","");
$blockServicos1->cellRow("Prioridade :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectStatus","","","","");
$blockServicos1->closeRow();

$blockServicos1->openRow("on");
$blockServicos1->cellRow("Prioridade :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectPrioridades","","","","");
$blockServicos1->cellRow("Área Responsável :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectAreaResponsavel","","","","");
$blockServicos1->closeRow(); 
		
$blockServicos1->openRow("on");
$blockServicos1->cellRow("Unidade :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectUnidades","","","","");
$blockServicos1->cellRow("&nbsp;","","","","");
$blockServicos1->cellRow("","","","","");
$blockServicos1->closeRow(); 
		
$blockServicos1->openRow("on");
$blockServicos1->cellRow("Gestor :&nbsp;","","","","","3");
$blockServicos1->cellRow("$selectGestores","","","","");
$blockServicos1->cellRow("&nbsp;","","","","");
$blockServicos1->cellRow("&nbsp;","","","","");
$blockServicos1->closeRow(); 	
...
$blockServicos1->openRow("on");
$blockServicos1->cellRow("<strong>Ano da Solicitação da Demanda :</strong>","","","","4","2");
$blockServicos1->closeRow();

$blockServicos1->openRow("on");
$blockServicos1->cellRow("$selectAnos","","","","4","2");
$blockServicos1->closeRow(); 
...
$blockServicos1->openRow("on");
$blockServicos1->cellRow("<input type='submit' name='Save' value='Filtrar'>","","","","4","2");
$blockServicos1->closeRow(); 

$blockServicos1->closeContent();
$blockServicos1->closeForm();
//FIM PRIMEIRO FORMULARIO

//INICIO SEGUNDO FORMULARIO
$blockServicos2->openForm("reportsFrameGrafico.php");
$blockServicos2->openContent();

$blockServicos2->openRow("on");
$blockServicos2->cellRow("<input type=\"hidden\" name=\"tp\" value=\"servicos\">
				  <div align=\"right\">Área Responsável :</div>","","","","");
$blockServicos2->cellRow("$selectAreaResponsavel","240","","","");
$blockServicos2->cellRow("<div align=\"right\">Ano :&nbsp;</div>","","","","");
$blockServicos2->cellRow("$selectAnos","","","","");
$blockServicos2->cellRow("<input type='submit' name='Save' value='Gerar Gráfico'>","100","","","");
$blockServicos2->closeRow(); 

$blockServicos2->closeContent();
$blockServicos2->closeForm();
?>