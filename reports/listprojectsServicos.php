...
if ($_REQUEST['uns'] != "") {
	$_SESSION['id_unidade'] = $_REQUEST['uns'];
} else {
	$_SESSION['id_unidade'];
}
...
$blockPage->itemBreadcrumbs($strings["demandas"]);
if ($typeProjects == "inactive") {
	$blockPage->itemBreadcrumbs($blockPage->buildLink("../reports/listprojects.php?typeProjects=active",$strings["active"],in)." | ".$strings["inactive"]);
} else if ($typeProjects == "active") {
	$blockPage->itemBreadcrumbs($strings["active"]." | ".$blockPage->buildLink("../reports/listprojects.php?typeProjects=inactive",$strings["inactive"],in));
}
...
$block1->form = "saP";
$block1->openForm("../reports/listprojectsServicos.php?typeProjects=$typeProjects&" . session_name() . "=" . session_id() . "#" . $block1->form . "Anchor");
...
if ($_SESSION['id_ano'] != '') {
	...
	$where = "WHERE $args = " . $_SESSION['id_ano'] . " ";
}
...
$tmpqueryy = "";
$tmpqueryy .= "$where AND pro.tipo_demanda IN ('1','2','3') ORDER BY pro.id DESC";
...
if ($comptListProjects != 0) {
	...
	for ($i = $borne1; $i < $borne1 + $tot; $i++) {
		...
		$block1->cellRow($blockPage->buildLink("../reports/relProjectsServicosPDF.php?id=" . $listProjects->pro_id[$i], $listProjects->pro_name[$i], inblank), '32%');
