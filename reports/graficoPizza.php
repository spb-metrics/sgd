...
$op = $_REQUEST['op'];
	...
	if ($op == 0 || $op == '0' || $op == "") {
		$ano = $_REQUEST['an'];
		ImageString($imagem, 5, 200, 3, "Gráfico Geral de Demandas", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);

		//ANO - Contar Numero de Demandas Novos Projetos
		$porAno_Projetos = new request();
		$tmpquery = " WHERE $years = '".$ano."' ";	
		$tmpquery.= " AND pro.tipo_demanda = '1'";
		$porAno_Projetos->openProjects($tmpquery);
		$corretiva = count($porAno_Projetos->pro_id);

		//ANO - Contar Numero de Demandas Corretivas]
		$porAno_Corretivas = new request();
		$tmpquery = " WHERE $years = '".$ano."' ";	
		$tmpquery.= " AND pro.tipo_demanda = '2'";	
		$porAno_Corretivas->openProjects($tmpquery);
		$evolutiva = count($porAno_Corretivas->pro_id);
		
		//ANO - Contar Numero de Demandas Evolutivas
		$porAno_Evolutivas = new request();
		$tmpquery = " WHERE $years = '".$ano."' ";	
		$tmpquery.= " AND pro.tipo_demanda = '0'";	
		$porAno_Evolutivas->openProjects($tmpquery);
		$novoProduto = count($porAno_Evolutivas->pro_id);
		
	} elseif ($op == 1) {
		$id_unidade = $_REQUEST['ung'];
		$ano = $_REQUEST['an'];

		//UNIDADE - Pegar sigla da Unidade
		$Unidade = new request();
		$tmpquery = " WHERE org.id = '".$id_unidade."' ";	
		$Unidade->openOrganizations($tmpquery);
		$siglaUnidade = $Unidade->org_name[0];

		ImageString($imagem, 5, 220, 3, "Demanda por Unidade", $preto);
		ImageString($imagem, 4, 8, 25, "Unidade: $siglaUnidade", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);
		
		//UNIDADE - Contar Numero de Demandas Novo Projeto
		$porUni_Projetos = new request();
		$tmpquery = "WHERE pro.organization = '".$id_unidade."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '1' ";	
		$porUni_Projetos->openProjects($tmpquery);
		$corretiva = count($porUni_Projetos->pro_id);
	
		//UNIDADE - Contar Numero de Demandas Corretivas		
		$porUni_Corretivas = new request();
		$tmpquery = "WHERE pro.organization = '".$id_unidade."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '2' ";	
		$porUni_Corretivas->openProjects($tmpquery);
		$evolutiva = count($porUni_Corretivas->pro_id);

		//UNIDADE - Contar Numero de Demandas Evolutivas		
		$porUni_Evolutivas = new request();
		$tmpquery = "WHERE pro.organization = '".$id_unidade."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '0' ";	
		$porUni_Evolutivas->openProjects($tmpquery);
		$novoProduto = count($porUni_Evolutivas->pro_id);

	} elseif ($op == 2) {
		$id_sistema = $_REQUEST['sig'];
		$ano = $_REQUEST['an'];

		//SISTEMA - Pegar nome do Sistema		
		$Sistema = new request();
		$tmpquery = "AND ss.id = '".$id_sistema."' ";	
		$Sistema->openSisunio($tmpquery);
		$nomeSistema = $Sistema->sis_nome[0];
		
		ImageString($imagem, 5, 220, 3, "Demanda por Sistema", $preto);
		ImageString($imagem, 4, 8, 25, "Sistema: $nomeSistema", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);

		//SISTEMA - Contar Numero de Demandas Novo Projeto
		$porSis_Projetos = new request();
		$tmpquery = "WHERE pro.id_sistema = '".$id_sistema."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '1' ";	
		$porSis_Projetos->openProjects($tmpquery);
		$corretiva = count($porSis_Projetos->pro_id);
		
		//SISTEMA - Contar Numero de Demandas Corretivas
		$porSis_Corretiva = new request();
		$tmpquery = "WHERE pro.id_sistema = '".$id_sistema."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '2' ";	
		$porSis_Corretiva->openProjects($tmpquery);
		$evolutiva = count($porSis_Corretiva->pro_id);

		//SISTEMA - Contar Numero de Demandas Evolutivas
		$porSis_Evolutiva = new request();
		$tmpquery = "WHERE pro.id_sistema = '".$id_sistema."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '0' ";	
		$porSis_Evolutiva->openProjects($tmpquery);
		$novoProduto = count($porSis_Evolutiva->pro_id);
	
	} elseif ($op == 3) {
		$id_gestor = $_REQUEST['gsg'];
		$ano = $_REQUEST['an'];

		//GESTOR - Pegar nome do Gestor		
		$Gestor = new request();
		$tmpquery = "AND SG.no_grupo = 'sgd_gestor' ";
		$tmpquery.= "AND SUG.st_ativo = 'S' ";	
		$tmpquery.= "AND SU.id = '".$id_gestor."'";	
		$Gestor->openSousuario($tmpquery);
		$nomeGestor = $Gestor->so_nome[0];
		
		ImageString($imagem, 5, 220, 3, "Demanda por Gestor", $preto);
		ImageString($imagem, 4, 8, 25, "Gestor: $nomeGestor", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);
				   
		//GESTOR - Contar Numero de Demandas Novo Projeto
		$porGes_Projetos = new request();
		$tmpquery = "WHERE pro.owner = '".$id_gestor."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '1' ";	
		$porGes_Projetos->openProjects($tmpquery);
		$corretiva = count($porGes_Projetos->pro_id);
		
		//GESTOR - Contar Numero de Demandas Corretiva
		$porGes_Corretiva = new request();
		$tmpquery = "WHERE pro.owner = '".$id_gestor."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '2' ";	
		$porGes_Corretiva->openProjects($tmpquery);
		$evolutiva = count($porGes_Corretiva->pro_id);
		
		//GESTOR - Contar Numero de Demandas Evolutiva
		$porGes_Evolutiva = new request();
		$tmpquery = "WHERE pro.owner = '".$id_gestor."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '0' ";	
		$porGes_Evolutiva->openProjects($tmpquery);
		$novoProduto = count($porGes_Evolutiva->pro_id);
		
	}elseif ($op == 4) {
		$id_gerproj = $_REQUEST['grg'];
		$ano = $_REQUEST['an'];
		
		//GERENTE - Pegar nome do Gerente		
		$Gerente = new request();
		$tmpquery = "AND SG.no_grupo = 'sgd_gerente_projeto' ";
		$tmpquery.= "AND SU.id = '".$id_gerproj."'";	
		$Gerente->openSousuario($tmpquery);
		$nomeGerente = $Gerente->so_nome[0];

		ImageString($imagem, 5, 220, 3, "Demanda por Gerente de Projeto", $preto);
		ImageString($imagem, 4, 8, 25, "Gerente: $nomeGerente", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);

		//GERENTE - Contar Numero de Demandas Novo Projeto
		$porGer_Projetos = new request();
		$tmpquery = "WHERE pro.id_ger_pro = '".$id_gerproj."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '1' ";	
		$porGer_Projetos->openProjects($tmpquery);
		$corretiva = count($porGer_Projetos->pro_id);
				
		//GERENTE - Contar Numero de Demandas Corretiva
		$porGer_Corretiva = new request();
		$tmpquery = "WHERE pro.id_ger_pro = '".$id_gerproj."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '2' ";	
		$porGer_Corretiva->openProjects($tmpquery);
		$evolutiva = count($porGer_Corretiva->pro_id);				
				   
		//GERENTE - Contar Numero de Demandas Evolutiva
		$porGer_Evolutiva = new request();
		$tmpquery = "WHERE pro.id_ger_pro = '".$id_gerproj."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '0' ";	
		$porGer_Evolutiva->openProjects($tmpquery);
		$novoProduto = count($porGer_Evolutiva->pro_id);
	
	}elseif ($op == 5) {
		$id_analistrel = $_REQUEST['anr'];
		$ano = $_REQUEST['an'];
		
		//ANALISTA RELACIONAMENTO - Pegar nome do Analista		
		$AnalistRel = new request();
		$tmpquery = "AND SG.no_grupo = 'sgd_analista_relat' ";
		$tmpquery.= "AND SU.id = '".$id_analistrel."'";
		$AnalistRel->openSousuario($tmpquery);
		$nomeAnalistRel = $AnalistRel->so_nome[0];

		ImageString($imagem, 5, 190, 3, "Demanda por Analista de Relacionamento", $preto);
		ImageString($imagem, 4, 8, 25, "Analista: $nomeAnalistRel", $preto);
		ImageString($imagem, 4, 8, 40, "Ano: $ano", $preto);

		//ANALISTA RELACIONAMENTO - Contar Numero de Demandas Novo Projeto
		$porAna_Projetos = new request();
		$tmpquery = "WHERE pro.id_analista = '".$id_analistrel."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '1' ";	
		$porAna_Projetos->openProjects($tmpquery);
		$corretiva = count($porAna_Projetos->pro_id);
				
		//ANALISTA RELACIONAMENTO - Contar Numero de Demandas Corretiva
		$porAna_Corretiva = new request();
		$tmpquery = "WHERE pro.id_analista = '".$id_analistrel."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '2' ";	
		$porAna_Corretiva->openProjects($tmpquery);
		$evolutiva = count($porAna_Corretiva->pro_id);				
				   
		//ANALISTA RELACIONAMENTO - Contar Numero de Demandas Evolutiva
		$porAna_Evolutiva = new request();
		$tmpquery = "WHERE pro.id_analista = '".$id_analistrel."' ";	
		$tmpquery.= "AND $years = '".$ano."' ";	
		$tmpquery.= "AND pro.tipo_demanda = '0' ";	
		$porAna_Evolutiva->openProjects($tmpquery);
		$novoProduto = count($porAna_Evolutiva->pro_id);
	}     

	$num_linhas = 3; 
	for($i=0 ; $i<$num_linhas; $i++) { 
		$dados = array('Novo produto', 'Corretiva', 'Evolutiva');
		$valores = array($novoProduto, $corretiva, $evolutiva);
		$i++;
	}

	if($valores != array (0, 0, 0)){
	  $total = 0;
	...
	ImageString($imagem, 3, 535, 300, "Total de Demandas: $total", $preto);
	$raio = $diametro/2;
	...
	for($i=0 ; $i<$num_linhas; $i++) {
		...
		$x_texto = $centrox + ($raio * cos(deg2rad($angulo_meio))) * 1.3;
		$y_texto = $centroy + (- $raio * sin(deg2rad($angulo_meio))) * 1.1;
	...
	if($exibir_legenda=="sim") {
		...
		ImageString($imagem, 15, 250, 100, "Total de Demanda: 0", $preto);
	}
