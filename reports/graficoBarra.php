...
function DemandasNovosSistemas($tempsql,$mes) {
	$years = chamaDataFormtDB('1');
	$month = chamaDataFormtDB('2');
	$ano = $_REQUEST['an'];
	$tmpquery1 = "WHERE pro.tipo_demanda = '0'  
				  AND $month = '$mes'  
				  AND $years = '".$ano."'  
				  $tempsql 
				  ORDER by pro.created";

	$novospr = open($tmpquery1);
	
	return $novospr;
}

function DemandasCorretivas($tempsql,$mes) {
	$years = chamaDataFormtDB('1');
	$month = chamaDataFormtDB('2');
	$ano = $_REQUEST['an'];

	$tmpquery2 = "WHERE pro.tipo_demanda = '1'  
				  AND $month = '$mes'  
				  AND $years = '".$ano."' 
				  $tempsql 
				  ORDER by pro.created";
	
	$correti = open($tmpquery2);
	return $correti;
}

function DemandasEvolutivas($tempsql,$mes) {
	$years = chamaDataFormtDB('1');
	$month = chamaDataFormtDB('2');	
	$ano = $_REQUEST['an'];

	$tmpquery3 = "WHERE pro.tipo_demanda = '2'  
				  AND $month = '$mes'  
				  AND $years = '".$ano."' 
				  $tempsql 
				  ORDER by pro.created";
					
	$evoluti = open($tmpquery3);
	return $evoluti;
}
...
function open($sql) {
	$request = new request();
	$request->openProjects($sql);
	$cont = count($request->pro_id);
	...
	$cores_linha = array ($azul, $verde, $vermelho, $violeta, $sienna, $verde2, $amarelo, $coral);
	$meses = array ("01","02","03","04","05","06","07","08","09","10","11","12");

	$tipo_demanda = 3;
	for($i=0 ; $i < $tipo_demanda; $i++) {
		$texto_linha = array("0", "1", "2");  
	}
	
	$numero_mes = 12;
	for($i=0 ; $i < $numero_mes; $i++){
		$texto_coluna = array("01", "02", "03","04", "05", "06","07", "08", "09", "10", "11", "12"); 
		$texto_coluna2 = array("JAN", "FEV", "MAR","ABR", "MAI", "JUN","JUL", "AGO", "SET", "OUT", "NOV", "DEZ"); 
	}

	if ($op == 0 || $op == '0' || $op == "") {
		$ano = $_REQUEST['an'];
		
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Gráfico Geral de Demandas";

		//GERAL - Contar Numero de Demandas Geral
		$tmpquery = "WHERE pro.tipo_demanda = '".$tipo."'
					 $month = '$mes'  
					 $years = '".$ano."' 
					 $tempsql 
					 ORDER by pro.created";
		...
		ImageString($imagem, 4, 8, 15, "Unidade: $siglaUnidade", $preto);
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Unidade";
		$tmpsql = "AND pro.organization = '".$id_unidade."' ";
		
		for ($i = 0;$i < 12;$i++) {
			$NovosSistemas[] = DemandasNovosSistemas($tmpsql,$meses[$i]);
			$Corretivas[] 	 = DemandasCorretivas($tmpsql,$meses[$i]);
			$Evolutivas[]	 = DemandasEvolutivas($tmpsql,$meses[$i]);			

			$valores = array_merge($NovosSistemas,$Corretivas,$Evolutivas);
		}
	} elseif ($op == 2) {
		...
		ImageString($imagem, 4, 8, 15, "Sistema: $nomeSistema", $preto);
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Sistema";
		
		$tmpsql = "AND id_sistema = '".$id_sistema."' ";
		...
		ImageString($imagem, 4, 8, 15,"Gestor: $nomeGestor", $preto);
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Gestor";
		
		$tmpsql = "AND pro.owner = '".$id_gestor."' ";
		...
		ImageString($imagem, 4, 8, 15,"Gerente de Projeto: $nomeGerente", $preto);
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Gerente de Projeto";
		
		$tmpsql = "AND pro.id_ger_pro = '".$id_grproj."' ";
		...
		ImageString($imagem, 4, 8, 15,"Analista de Relacionamento: $nomeAnalistRel", $preto);
		ImageString($imagem, 4, 8, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Analista de Relacionamento";

		$tmpsql = "AND pro.id_analista = '".$id_analista."' ";
	...
	ImageString($imagem, 5, 270, 3, $titulo, $preto);
	...
	if($exibir_legenda=="sim") { 
		$separat = " ";
		$maior_tamanho = 0;
		$texto_linha = array("Novo Sistema", "Corretiva", "Evolutiva");
	...
	$t = array_sum($valores);
	ImageString($imagem, 3, 535, 350,"Total de Demandas: $t", $preto);
	
	$j = $valores[0] + $valores[12] + $valores[24] + $valores[36] + $valores[48] + $valores[60] + $valores[72] + $valores[84];
	ImageString($imagem, 4, 535, 110,"Total Janeiro: $j", $preto);
	$f = $valores[1] + $valores[13] + $valores[25] + $valores[37] + $valores[49] + $valores[61] + $valores[73] + $valores[85];
	ImageString($imagem, 4, 535, 130,"Total Fevereiro: $f", $preto);
	$m = $valores[2] + $valores[14] + $valores[26] + $valores[38] + $valores[50] + $valores[62] + $valores[74] + $valores[86];
	ImageString($imagem, 4, 535, 150,"Total Março: $m", $preto);
	$a = $valores[3] + $valores[15] + $valores[27] + $valores[39] + $valores[51] + $valores[63] + $valores[75] + $valores[87];
	ImageString($imagem, 4, 535, 170,"Total Abril: $a", $preto);
	$ma = $valores[4] + $valores[16] + $valores[28] + $valores[40] + $valores[52] + $valores[64] + $valores[76] + $valores[88];
	ImageString($imagem, 4, 535, 190,"Total Maio: $ma", $preto);
	$ju = $valores[5] + $valores[17] + $valores[29] + $valores[41] + $valores[53] + $valores[65] + $valores[77] + $valores[89];
	ImageString($imagem, 4, 535, 210,"Total Junho: $ju", $preto);
	$jul = $valores[6] + $valores[18] + $valores[30] + $valores[42] + $valores[54] + $valores[66] + $valores[78] + $valores[90];
	ImageString($imagem, 4, 535, 230,"Total Julho: $jul", $preto);
	$ag = $valores[7] + $valores[19] + $valores[31] + $valores[43] + $valores[55] + $valores[67] + $valores[79] + $valores[91];
	ImageString($imagem, 4, 535, 250,"Total Agosto: $ag", $preto);
	$s = $valores[8] + $valores[20] + $valores[32] + $valores[44] + $valores[56] + $valores[68] + $valores[80] + $valores[92];
	ImageString($imagem, 4, 535, 270,"Total Setembro: $s", $preto);
	$o = $valores[9] + $valores[21] + $valores[33] + $valores[45] + $valores[57] + $valores[69] + $valores[81] + $valores[93];
	ImageString($imagem, 4, 535, 290,"Total Outubro: $o", $preto);
	$n = $valores[10] + $valores[22] + $valores[34] + $valores[46] + $valores[58] + $valores[70] + $valores[82] + $valores[94];
	ImageString($imagem, 4, 535, 310,"Total Novembro: $n", $preto);
	$d = $valores[11] + $valores[23] + $valores[35] + $valores[47] + $valores[59] + $valores[71] + $valores[83] + $valores[95];
	ImageString($imagem, 4, 535, 330,"Total Dezembro: $d", $preto);
