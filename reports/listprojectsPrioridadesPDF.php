...
$id_gestor = $_REQUEST['gsst'];
$id_sistema = $_REQUEST['sist'];
...
// GERENTE DE PROJETO
	$IDGerente = new request();
	$tmpquery = "";	
	$tmpquery = "WHERE pro.id = ".$id_sistema." ";
	$IDGerente->openProjects($tmpquery);
	$IDGerente = $IDGerente->pro_mem_idgpro[0];
	
	if ($IDGerente) {	
		$Gerente = new request();
		$tmpquery = "";
		$tmpquery = "WHERE mem.id = ".$IDGerente." ";
		$Gerente->openMembers($tmpquery);
		$Gerente = $Gerente->mem_name[0];
	} else {
		$Gerente = "Em Definição";
	}
...
// GESTOR DO PROJETO
	...
	$Gestor = $Gestor->so_nome[0];

// NOME DO SISTEMA
	$Sistema = new request();
	$tmpquery = "";
	$tmpquery = "WHERE ss.id = ".$id_sistema." ";	
	$Sistema->openSosistema($tmpquery);
	$Sistema = $Sistema->sis_nome[0];
	 
// QUANTIDADE DE DEMANDAS
		$Geral = new request();
		$tmpquery = "";
		$tmpquery = "WHERE pro.status  <> 10 ";
		$tmpquery.= "AND pro.status  <> 11 ";
		$tmpquery.= "AND pro.id = ".$id_sistema." ";
		$tmpquery.= "AND pro.owner = ".$id_gestor." ";
		$Geral->openProjects($tmpquery);
		$countDemandas = count($Geral->pro_id);
	...
	$pdf->Cell(180,17,"LISTA DE PRIORIDADES",0,2,'C',1);
	
	$pdf->SetXY(11,40);
	$pdf->SetTextColor(170,85,85); //Cor da fonte vermelha
	$pdf->SetFont('helvetica','',11);
	$pdf->Cell(180,17,"Gestor: ".$Gestor,0,2,'L',1);
	$pdf->SetX(0);
	...
	$pdf->Cell(11,5,"Gerente de projeto:  ",0,0,'L',0);
	$pdf->SetX(38);
	$pdf->SetFont('helvetica','',8);
	$pdf->Cell(65,5,$Gerente,0,0,'L',0); 
	$pdf->Ln();
	
	$pdf->SetX(11);
	$pdf->SetFont('helvetica','B',8); 
	$pdf->Cell(87,5,"Sistema: ",1,0,'L',0);
	$pdf->SetX(24);
	$pdf->SetFont('helvetica','',8); 
	$pdf->Cell(85,5,$Sistema,1,2,'L',0); 
	
	$pdf->SetX(11);
	$pdf->SetFont('helvetica','B',8); 
	$data = strftime("%d/%m/%Y");
	$pdf->Cell(55,5,"Data de impressão:",1,0,'L',0);
	$pdf->SetX(38);
	$pdf->SetFont('helvetica','',8); 
	$pdf->Cell(50,5,$data,1,0,'L',0);
	$pdf->Ln();

	$pdf->Cell(190,2,"________________________________________________________________",0,2,'L',0);
	$pdf->Ln();
        
    if($countDemandas > 0){
     	$pdf->SetFillColor(40, 88, 101); 
        $pdf->SetDrawColor(40, 88, 101);
        $pdf->SetTextColor(255, 255, 255);  
       	
       	$pdf->Rect(11, 74, 10, 5,FD);
       	
       	$pdf->Rect(20, 74, 10, 5,FD);
    	$pdf->Rect(30, 74, 80, 5,FD);
    	$pdf->Rect(100, 74, 20, 5,FD);
    	$pdf->Rect(120, 74, 30, 5,FD);
    	$pdf->Rect(150, 74, 20, 5,FD);
    	$pdf->Rect(170, 74, 20, 5,FD);
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(23.5, 77.5, 'ID');   
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(60, 77.5, 'TÍTULO');
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(101, 77.5, 'PRIORIDADE');
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(130, 77.5, 'STATUS');
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(151, 77.5, 'DATA INÍCIO');
    	$pdf->SetFont('helvetica','B',8);
    	$pdf->Text(173, 77.5, 'DATA FIM');
        $pdf->SetY(80);

		for($i = 0; $i < $countDemandas; $i++) { 
			...
			$pdf->SetX(20);
			$pdf->SetDrawColor(255,255,255);
			$pdf->Cell(10,5,$Geral->pro_id[$i],1,0,'C',0);
			$pdf->SetX(30); 
			$pdf->Cell(70,5,substr($Geral->pro_name[$i], 0, 100),1,0,'L',0);
			$pdf->SetX(100);
			$PRIORIDADE = $Geral->pro_priority[$i];

			if($PRIORIDADE == 2)	  $PRIORIDADE = "Baixa";
			elseif ($PRIORIDADE == 3) $PRIORIDADE = "Média";
			elseif ($PRIORIDADE == 5) $PRIORIDADE = "Alta";

			$pdf->Cell(20,5,$PRIORIDADE,1,0,'C',0);
			$pdf->SetX(120);
			$STATUS = $Geral->pro_status[$i];

			if($STATUS == 0)	  $STATUS = "Val. Cliente";
			elseif($STATUS == 1)  $STATUS = "Em Analise";
			elseif($STATUS == 2)  $STATUS = "Suspensa";
			elseif($STATUS == 3)  $STATUS = "Aberto";
			elseif($STATUS == 4)  $STATUS = "Val. Relacionamento";
			elseif($STATUS == 5)  $STATUS = "Validado TI";
			elseif($STATUS == 6)  $STATUS = "Em Andamento";
			elseif($STATUS == 7)  $STATUS = "Aguard. Homologação";
			elseif($STATUS == 8)  $STATUS = "Aceito pelo Cliente";
			elseif($STATUS == 9)  $STATUS = "Em Homologação";
			elseif($STATUS == 12) $STATUS = "Pendente";

			$pdf->Cell(30,5,$STATUS,1,0,'C',0);
			$pdf->SetX(150);
			$pdf->Cell(20,5,invert_data($res4['DI'][$i]),1,0,'C',0);
			$pdf->SetX(170);
			$pdf->Cell(20,5,invert_data($res4['DF'][$i]),1,0,'C',0);
			$pdf->Ln();
		}
	} else { 
        $pdf->SetFillColor(255, 255, 255);  
		$pdf->SetTextColor(0, 0, 0); 
		$pdf->SetDrawColor(255,255,255); 
        $pdf->Rect(20, 74, 70, 5,FD);
        $pdf->SetFont('helvetica','B',8);
    	$pdf->Text(23.5, 77.5, 'Não existe demanda ativa cadastrada para este Sistema!');   
	}    
