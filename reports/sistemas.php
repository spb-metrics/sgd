...
if ($id_gestor == '0' || $id_gestor == 0 || $id_gestor == "") {
	$tmp_sistema = "WHERE ss.nu_seq_gestor_aplicacao = 'xxx' ";
} else {
	$tmp_sistema = "WHERE ss.nu_seq_gestor_aplicacao = '" . $id_gestor . "'";
}
...
for ($i = 0; $i < $contSistema; $i++) {
	//OCIExecute($result);
	$codigo = $sel_Sistema->sis_id[$i];
	$descricao = $sel_Sistema->sis_nome[$i];
