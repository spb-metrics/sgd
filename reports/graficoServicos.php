...
$arearesp = $as;
	...
	if($arearesp == 5) $arearesp = "Produção";
	elseif ($arearesp == 6) $arearesp = "Web";
	elseif ($arearesp == 7) $arearesp = "Gestão de Conhecimento";
	elseif ($arearesp == 8) $arearesp = "Infraestrutura";
	...
	$verde 		= ImageColorAllocate($imagem, 0,255,127);
	...
	if ($as == 0 || $as == '0' || $as == "") {
		...
		$titulo = "Gráfico Geral de Demandas de Serviço";

		//AREA RESPONSAVEL - Contar Numero de Demandas Geral
		$Geral = new request();
		$tmpquery = "WHERE pro.PHASE_SET = '".$tipo."'
					 AND $month = '$mes'
					 AND $years = '".$ano."'";
		...
		$unidade = $Unidade->org_name[0];

        ImageString($imagem, 4, 3, 15, "Área Responsável: $arearesp", $preto);
		ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
		$titulo = "Demanda por Área Responsável";

		//AREA RESPONSAVEL - Contar Numero de Demandas da area 5
		$porArea5 = new request();
		$tmpquery = "WHERE pro.PHASE_SET = '".$as."'	
					 AND pro.PHASE_SET = '".$tipo."'
					 $month = '$mes'
					 $years = '".$ano."'";

		$porArea5->openProjects($tmpquery);
		$row = count($porArea5->pro_id);
		...
		//AREA RESPONSAVEL - Contar Numero de Demandas da area 6
		$porArea6 = new request();
		$tmpquery = "WHERE pro.PHASE_SET = '".$as."'	
					 AND pro.PHASE_SET = '".$tipo."'
					 $month = '$mes'
					 $years = '".$ano."'";

		$porArea6->openProjects($tmpquery);
		$row = count($porArea6->pro_id);
			...
			//AREA RESPONSAVEL - Contar Numero de Demandas da area 7
			$porArea7 = new request();
			$tmpquery = "WHERE pro.PHASE_SET = '".$as."'	
						 AND pro.PHASE_SET = '".$tipo."'
						 $month = '$mes'
						 $years = '".$ano."'";
	
			$porArea7->openProjects($tmpquery);
			$row = count($porArea7->pro_id);
			...
			//AREA RESPONSAVEL - Contar Numero de Demandas da area 8
			$porArea8 = new request();
			$tmpquery = "WHERE pro.PHASE_SET = '".$as."'	
						 AND pro.PHASE_SET = '".$tipo."'
						 $month = '$mes'
						 $years = '".$ano."'";
	
			$porArea8->openProjects($tmpquery);
			$row = count($porArea8->pro_id);
		}
	    if($row > 0) $valores[] = $row;        
        else $valores[] = 0;
	}	 
 }
...
ImageString($imagem, 5, 160, 3, $titulo, $preto);
...
{
    $maior_tamanho = 0;
	$texto_linha = array("Produção", "Web", "Gest. Conhecimento", "Infraestrutura");
    for($i=0 ; $i<$numero_linhas; $i++)
        if(strlen($texto_linha[$i])>$maior_tamanho)
            $maior_tamanho = strlen($texto_linha[$i]);

    $x_inicio_legenda = $lx - $largura_fonte * ($maior_tamanho+2);
    $y_inicio_legenda = $ly;

    $x_fim_legenda = $lx;
    $y_fim_legenda = $ly + $numero_linhas * ($altura_fonte + $espaco_entre_linhas) + 1*$margem_vertical;
...
$j = $valores[0] + $valores[12] + $valores[24] + $valores[36];
ImageString($imagem, 4, 535, 125,"Total Janeiro: $j", $preto);
$f = $valores[1] + $valores[13] + $valores[25] + $valores[37];
ImageString($imagem, 4, 535, 145,"Total Fevereiro: $f", $preto);
$m = $valores[2] + $valores[14] + $valores[26] + $valores[38];
ImageString($imagem, 4, 535, 165,"Total Março: $m", $preto);
$a = $valores[3] + $valores[15] + $valores[27] + $valores[39];
ImageString($imagem, 4, 535, 185,"Total Abril: $a", $preto);
$ma = $valores[4] + $valores[16] + $valores[28] + $valores[40];
ImageString($imagem, 4, 535, 205,"Total Maio: $ma", $preto);
$ju = $valores[5] + $valores[17] + $valores[29] + $valores[41];
ImageString($imagem, 4, 535, 225,"Total Junho: $ju", $preto);
$jul = $valores[6] + $valores[18] + $valores[30] + $valores[42];
ImageString($imagem, 4, 535, 245,"Total Julho: $jul", $preto);
$ag = $valores[7] + $valores[19] + $valores[31] + $valores[43];
ImageString($imagem, 4, 535, 265,"Total Agosto: $ag", $preto);
$s = $valores[8] + $valores[20] + $valores[32] + $valores[44];
ImageString($imagem, 4, 535, 285,"Total Setembro: $s", $preto);
$o = $valores[9] + $valores[21] + $valores[33] + $valores[45];
ImageString($imagem, 4, 535, 305,"Total Outubro: $o", $preto);
$n = $valores[10] + $valores[22] + $valores[34] + $valores[46];
ImageString($imagem, 4, 535, 325,"Total Novembro: $n", $preto);
$d = $valores[11] + $valores[23] + $valores[35] + $valores[47];
ImageString($imagem, 4, 535, 345,"Total Dezembro: $d", $preto);
