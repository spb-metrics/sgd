...
$years = chamaDataFormtDB('1');
$month = chamaDataFormtDB('2');
...
$violeta = ImageColorAllocate($imagem, 138, 43, 226);
$sienna = ImageColorAllocate($imagem, 160, 82, 45);
$verde2 = ImageColorAllocate($imagem, 0, 128, 128);
$amarelo = ImageColorAllocate($imagem, 255, 255, 0);
$coral = ImageColorAllocate($imagem, 255, 127, 80);
$brancosmoke = ImageColorAllocate($imagem, 245, 245, 245);
$vcinza = ImageColorAllocate($imagem, 119, 136, 153);
$thistle = ImageColorAllocate($imagem, 216, 191, 216);
$vaqua = ImageColorAllocate($imagem, 72, 209, 204);
$blavender = ImageColorAllocate($imagem, 230, 230, 250);
...
$data = date("d/m/y");
$hora = (date("H") - 3) . ":" . date("i:s");
$numero_mes = 1;

for ($i = 0; $i < $numero_mes; $i++) {
	$texto_coluna = array (
		$data
	);
}
...
for ($i = 0; $i < $tipo_demanda; $i++) {
	for ($j = 0; $j < $numero_mes; $j++) {
		$tipo = $texto_linha[$i];
		$mes = $texto_coluna[$j];

		if ($op == 15 || $op == '0' || $op == 0 || $op == "") {
			$ano = $_REQUEST['an'];

			if ($mes2 == 0) {
				ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
				if ($op != 15) {
					$titulo = "Gráfico Geral de Status";
				} else {
					$titulo = "Gráfico Geral de Abertas x Concluídas";
				}

				$Geral = new request();
				$tmpquery = "WHERE pro.status = '" . $tipo . "'
							 AND $years = '" . $ano . "'";
				
				$Geral->openProjects($tmpquery);
				$row = count($Geral->pro_id);

				ImageString($imagem, 3, 200, 370, "$data - $hora", $preto);

			} else {
				ImageString($imagem, 4, 3, 15, "Ano: $ano", $preto);
				ImageString($imagem, 4, 200, 370, "$m", $preto);
				$titulo = "Gráfico Geral de Abertas x Concluídas";
				$Geral = new request();
		...
		elseif ($op == 1) {
			$id_unidade = $_REQUEST['ungc'];
			$ano = $_REQUEST['an'];

			//UNIDADE - Pegar sigla da Unidade
			$Unidade = new request();
			$tmpquery = " AND org.id = '" . $id_unidade . "' ";
			$Unidade->openOrganizations($tmpquery);
			$siglaUnidade = $Unidade->org_name[0];

			ImageString($imagem, 4, 3, 15, "Unidade: $siglaUnidade", $preto);
			ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
			$titulo = "Status por Unidade";

			$porUnidade = new request();
			$tmpquery = "WHERE org.organization = '" . $id_unidade . "'	
						 AND pro.status = '" . $tipo . "'
						 AND $years = '" . $ano . "'";

			$porUnidade->openProjects($tmpquery);
			$row = count($porUnidade->pro_id);
			ImageString($imagem, 3, 200, 370, "$data - $hora", $preto);

		}
		elseif ($op == 2) {
			$id_sistema = $_REQUEST['sigc'];
			$ano = $_REQUEST['an'];

			$Sistema = new request();
			$tmpquery = "AND SG.id = '" . $id_sistema . "' ";
			$Sistema->openSistema($tmpquery);
			$nomeSistema = $Sistema->sis_nome[0];

			ImageString($imagem, 4, 3, 15, "Sistema: $nomeSistema", $preto);
			ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
			$titulo = "Status por Sistema";

			$porSistema = new request();
			$tmpquery = "WHERE id = '" . $id_sistema . "'	
						 AND pro.status = '" . $tipo . "'
						 AND $years = '" . $ano . "'";

			$porSistema->openProjects($tmpquery);
			$row = count($porSistema->pro_id);

			ImageString($imagem, 3, 200, 370, "$data - $hora", $preto);

		}
		elseif ($op == 3) {
			$id_gestor = $_REQUEST['gsgc'];
			$ano = $_REQUEST['an'];

			$Gestor = new request();
			$tmpquery = "AND SG.no_grupo = 'sgd_gestor' 
						 AND SUG.st_ativo = 'S' 	
						 AND SU.id = '" . $id_gestor . "'";
			$Gestor->openSousuario($tmpquery);
			$nomeGestor = $Gestor->so_nome[0];

			ImageString($imagem, 4, 3, 15, "Gestor: $nomeGestor", $preto);
			ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
			$titulo = "Demanda por Gestor";

			$porGestor = new request();
			$tmpquery = "WHERE pro.owner = '" . $id_gestor . "'	
						 AND pro.status = '" . $tipo . "'
						 AND $years = '" . $ano . "'";
			$porGestor->openProjects($tmpquery);
			$row = count($porGestor->pro_id);

			ImageString($imagem, 3, 200, 370, "$data - $hora", $preto);

		}
		elseif ($op == 4) {
			$id_grproj = $_REQUEST['grgc'];
			$ano = $_REQUEST['an'];

			$Gerente = new request();
			$tmpquery = "AND SG.no_grupo = 'sgd_gerente_projeto' 
						 AND SUG.st_ativo = 'S' 	
						 AND SU.id = '" . $id_grproj . "'";
			$Gerente->openSousuario($tmpquery);
			$nomeGerente = $Gerente->so_nome[0];

			ImageString($imagem, 4, 3, 15, "Gerente de Projeto: $nomeGerente", $preto);
			ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
			$titulo = "Demanda por Gerente de Projeto";

			$porGerente = new request();
			$tmpquery = "WHERE pro.id_ger_pro = '" . $id_grproj . "'	
						 AND pro.status = '" . $tipo . "'
						 AND $years = '" . $ano . "'";
			$porGerente->openProjects($tmpquery);
			$row = count($porGerente->pro_id);

			ImageString($imagem, 3, 200, 370, "$data - $hora", $preto);

		}
		elseif ($op == 5) {
			$id_analista = $_REQUEST['anrc'];
			$ano = $_REQUEST['an'];

			$AnalistRel = new request();
			$tmpquery = "AND SG.no_grupo = 'sgd_analista_relat' ";
			$tmpquery.= "AND SU.id = '" . $id_analista . "'";

			$AnalistRel->openSousuario($tmpquery);
			$nomeAnalistRel = $AnalistRel->so_nome[0];

			ImageString($imagem, 4, 3, 15, "Analista de Relacionamento: $nomeAnalistRel", $preto);
			ImageString($imagem, 4, 3, 30, "Ano: $ano", $preto);
			$titulo = "Demanda por Analista de Relacionamento";

			$porAnalista = new request();
			$tmpquery = "WHERE pro.id_analista = '" . $id_analista . "'	
						 AND pro.status = '" . $tipo . "'
						 AND $years = '" . $ano . "'";

			$porAnalista->openProjects($tmpquery);
			$row = count($porAnalista->pro_id);
...
if ($y_maximo < 1)
	$variacao = 0.1;
elseif ($y_maximo < 10) $variacao = 1;
elseif ($y_maximo < 2 * $fator) $variacao = $fator / 5;
elseif ($y_maximo < 5 * $fator) $variacao = $fator / 2;
elseif ($y_maximo < 10 * $fator) $variacao = $fator;
...
ImageString($imagem, 5, 180, 3, $titulo, $preto);
...
for ($i = 0; $i <= $num_pontos_eixo_y; $i++) {
	$posx = $inicio_grafico_x - (strlen($valor) + 2) * 6;
...
for ($i = 0; $i < $numero_colunas; $i++) {
	...
	ImageString($imagem, 2, $pos_label_x, $pos_label_y, $texto_coluna2[$i], $preto);
...
if ($op != 6) {
	$a = $valores[0] + $valores[13] + $valores[26] + $valores[39] + $valores[52] + $valores[65] + $valores[78] + $valores[91] + $valores[104] + $valores[117] + $valores[130] + $valores[143] + $valores[156];
	$b = $valores[1] + $valores[14] + $valores[27] + $valores[40] + $valores[53] + $valores[66] + $valores[79] + $valores[92] + $valores[105] + $valores[118] + $valores[131] + $valores[144] + $valores[157];
	$c = $valores[2] + $valores[15] + $valores[28] + $valores[41] + $valores[54] + $valores[67] + $valores[80] + $valores[93] + $valores[106] + $valores[119] + $valores[132] + $valores[145] + $valores[158];
	$d = $valores[3] + $valores[16] + $valores[29] + $valores[42] + $valores[55] + $valores[68] + $valores[81] + $valores[94] + $valores[107] + $valores[120] + $valores[133] + $valores[146] + $valores[159];
	$e = $valores[4] + $valores[17] + $valores[30] + $valores[43] + $valores[56] + $valores[69] + $valores[82] + $valores[95] + $valores[108] + $valores[121] + $valores[134] + $valores[147] + $valores[160];
	$f = $valores[5] + $valores[18] + $valores[31] + $valores[44] + $valores[57] + $valores[70] + $valores[83] + $valores[96] + $valores[109] + $valores[122] + $valores[135] + $valores[148] + $valores[161];
	$g = $valores[6] + $valores[19] + $valores[32] + $valores[45] + $valores[58] + $valores[71] + $valores[84] + $valores[97] + $valores[110] + $valores[123] + $valores[136] + $valores[149] + $valores[162];
	$h = $valores[7] + $valores[20] + $valores[33] + $valores[46] + $valores[59] + $valores[72] + $valores[85] + $valores[98] + $valores[111] + $valores[124] + $valores[137] + $valores[150] + $valores[163];
	$i = $valores[8] + $valores[21] + $valores[34] + $valores[47] + $valores[60] + $valores[73] + $valores[86] + $valores[99] + $valores[112] + $valores[125] + $valores[138] + $valores[151] + $valores[164];
	$j = $valores[9] + $valores[22] + $valores[35] + $valores[48] + $valores[61] + $valores[74] + $valores[87] + $valores[100] + $valores[113] + $valores[126] + $valores[139] + $valores[152] + $valores[165];
	$l = $valores[10] + $valores[23] + $valores[36] + $valores[49] + $valores[62] + $valores[75] + $valores[88] + $valores[101] + $valores[114] + $valores[127] + $valores[140] + $valores[153] + $valores[166];
	$m = $valores[11] + $valores[24] + $valores[37] + $valores[50] + $valores[63] + $valores[76] + $valores[89] + $valores[102] + $valores[115] + $valores[128] + $valores[141] + $valores[154] + $valores[167];
	$n = $valores[12] + $valores[25] + $valores[38] + $valores[51] + $valores[64] + $valores[77] + $valores[90] + $valores[103] + $valores[116] + $valores[129] + $valores[142] + $valores[155] + $valores[168];
...
if ($exibir_legenda == "sim") {
	...
	if ($op == 6) {
		for ($i = 0; $i < $tipo_demanda; $i++) {
			$TL[] = substr($resgp['NAME'][$i], 0, strpos($resgp['NAME'][$i], $separat));
			$texto_linha = $TL;
		}
	} else {
		$texto_linha = array (
	...
	for ($i = 0; $i < $numero_linhas; $i++) {
		...
		ImageString($imagem, $fonte, $x_pos, $y_pos, $texto_linha[$i] . ':' . ' ' . $tval[$i] . ' ', $preto);
...
$t = array_sum($valores);
if ($op != 15) {
	ImageString($imagem, 3, 520, 370, "Total de Demandas: $t", $preto);
} else {

	ImageString($imagem, 3, 520, 380, "Total de Demandas: $t", $preto);
	ImageString($imagem, 3, 520, 340, "Total de Abertas: $valab", $preto);
	ImageString($imagem, 3, 520, 360, "Total de Concluídas: $valcc", $preto);
